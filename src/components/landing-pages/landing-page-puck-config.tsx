...
function ActionField({ value, onChange, forms }: ActionFieldProps) {
  const action = value as LandingPageAction | undefined;
  const type = action?.type ?? "anchor";
  return <div className="grid gap-2">
    <label className="text-sm font-medium">按鈕動作</label>
...
const actionField = (forms: LandingPageFormReference[]) => ({ type: "custom" as const, label: "動作設定", render: (props: FieldProps) => <ActionField {...props} forms={forms} /> });
const styleField = {
...
export function createLandingPagePuckConfig(context: Pick<LandingPageRenderContext, "forms">): Config {
  ...
  const config: Config = {
    categories: {
      basic: { title: "基本內容", components: ["Heading", "Text", "Image", "Video", "Button"] },
      layout: { title: "版面配置", components: ["Section", "Columns"] },
      conversion: { title: "轉換模組", components: ["Hero", "Speaker", "Benefits", "Agenda", "Carousel", "Pricing", "Countdown", "Testimonials", "FAQ", "CTA"] },
    },
    root: { fields: { title: textField("頁面標題"), description: textField("SEO 描述", true), shareImage: textField("分享圖片 HTTPS 網址") }, render: ({ children }: { children: ReactNode }) => <main className="min-h-screen bg-white text-slate-950">{children}</main> },
    components: {
      ...
      Button: { label: "按鈕", fields: { label: textField("按鈕文字"), action }, defaultProps: { label: "立即行動", action: { type: "anchor", targetId: "register" } }, render: ({ label }) => <button type="button" className="rounded-xl bg-slate-950 px-5 py-3 font-bold text-white">{label}</button> },
      Section: { label: "區段（可巢狀拖放）", fields: { anchorId: textField("錨點 ID"), style: styleField, children: slot }, defaultProps: { children: [] }, render: ({ children }) => { const Children = children as SlotProps; return <section className="mx-auto my-5 max-w-6xl rounded-2xl border border-slate-200 p-5"><Children minEmptyHeight={96} /></section>; } },
      ...
      Hero: { label: "主視覺", fields: { eyebrow: textField("眉標"), title: textField("標題"), description: textField("說明", true), imageUrl: textField("背景圖片網址"), ctaLabel: textField("按鈕文字"), ctaAction: action }, defaultProps: { title: "讓你的活動被看見", description: "用清楚的內容，邀請對的人加入。", ctaLabel: "立即報名", ctaAction: { type: "anchor", targetId: "register" } }, render: ({ eyebrow, title, description, ctaLabel }) => <section className="rounded-3xl bg-slate-950 px-7 py-16 text-white">{eyebrow ? <p className="text-sm font-bold tracking-widest text-amber-300">{eyebrow}</p> : null}<h1 className="mt-3 text-4xl font-black">{title}</h1>{description ? <p className="mt-4 max-w-2xl whitespace-pre-line text-slate-200">{description}</p> : null}{ctaLabel ? <button type="button" className="mt-7 rounded-xl bg-white px-5 py-3 font-bold text-slate-950">{ctaLabel}</button> : null}</section> },
      ...
      Countdown: { label: "倒數計時", fields: { title: textField("標題"), mode: { type: "select", label: "倒數模式", options: [{ label: "固定截止時間", value: "fixed_date" }, { label: "依直播開播時間", value: "live_linked" }] }, targetAt: textField("固定截止時間（ISO 8601）"), expiredMessage: textField("結束訊息") }, defaultProps: { title: "報名即將截止", mode: "fixed_date", targetAt: "2030-01-01T00:00:00+08:00", expiredMessage: "活動已結束" }, render: ({ title, mode, targetAt }) => <section className="rounded-2xl bg-amber-50 p-6 text-center"><h2 className="text-2xl font-black">{title}</h2><time className="mt-3 block font-bold text-amber-900">{mode === "live_linked" ? "依直播開播時間倒數" : `截止：${targetAt}`}</time></section> },
      ...
      CTA: { label: "行動呼籲", fields: { title: textField("標題"), description: textField("說明", true), ctaLabel: textField("按鈕文字"), ctaAction: action }, defaultProps: { title: "準備好了嗎？", ctaLabel: "立即報名", ctaAction: { type: "anchor", targetId: "register" } }, render: ({ title, description, ctaLabel }) => <section className="rounded-3xl bg-amber-400 p-8 text-center"><h2 className="text-3xl font-black">{title}</h2>{description ? <p className="mt-3">{description}</p> : null}{ctaLabel ? <button type="button" className="mt-5 rounded-xl bg-slate-950 px-5 py-3 font-bold text-white">{ctaLabel}</button> : null}</section> },
    },
  };
  for (const component of Object.values(config.components) as Array<{ fields?: Record<string, unknown>; render: (props: Record<string, unknown>) => ReactElement }>) {
    ...
    component.render = (props) => <div className="landing-page-editor-block" style={editorStyle(props.style)}>{render(props)}</div>;
  }
  return config;
}
