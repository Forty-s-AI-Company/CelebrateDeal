...
export function FunnelStepPagesEditor({ state, disabled = false, onChange }: { state: FunnelStepPages; disabled?: boolean; onChange: (state: FunnelStepPages) => void }) {
  ...
  const addStep = () => {
    ...
    const added = addFunnelStepPage(state, { name: "新步驟", path: `step-${ordinal}`, type: newStepType, templateSource: newStepSource, ...(newStepSource === "template" && templateId ? { templateId } : {}) });
    if (!added.ok) return;
    const created = added.state.flow.steps.find((step) => !state.flow.steps.some((old) => old.id === step.id));
    if (!created || newStepSource !== "template" || !templateId) return commit(added);
    const page = added.state.pages[created.id];
    if (!page) return;
    const replaced = replaceFunnelStepPage(added.state, created.id, instantiateFunnelTemplate(templateId, page.id));
    if (replaced.ok) commit(replaced);
  };
  return <div className="grid min-h-0 grid-cols-[15rem_minmax(0,1fr)] overflow-hidden">
    ...
    <div className="min-h-0 overflow-auto"><FunnelPageEditor key={state.activeStepId} document={active.page} disabled={disabled || !active.editable} onChange={(page) => commit(replaceFunnelStepPage(state, active.step.id, page), false)} /></div>
  </div>;
}
