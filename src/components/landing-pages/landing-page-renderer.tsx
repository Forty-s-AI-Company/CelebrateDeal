...
function LandingPageBlock({ block, context }: { block: Block; context: LandingPageRenderContext }) {
  switch (block.type) {
    ...
    case "Countdown": {
      const cta = block.props.ctaLabel && block.props.ctaAction ? { label: block.props.ctaLabel, href: actionHref(block.props.ctaAction, context), pageId: context.pageId } : undefined;
      const expiredRedirectHref = block.props.expiredRedirect ? actionHref(block.props.expiredRedirect, context) : undefined;
      return <section className={`${classes(block.props.style)} mx-auto rounded-2xl border border-amber-200/70 bg-gradient-to-b from-amber-50 to-orange-50/70 p-6 text-center shadow-[0_1px_3px_rgba(0,0,0,0.05)]`} style={styleValues(block.props.style)}>{block.props.title ? <h2 className="text-2xl font-black">{block.props.title}</h2> : null}<LandingPageCountdown targetAt={block.props.mode === "live_linked" ? context.live?.scheduledAt : block.props.targetAt} expiredMessage={block.props.expiredMessage} cta={cta} expiredAction={block.props.expiredAction} expiredRedirectHref={expiredRedirectHref} /></section>;
    }
