...
export function CarouselBlock({ settings }: { settings: CarouselSliderBlock["settings"] }) {
  ...
  useEffect(() => {
    if (!settings.autoPlay || paused || reducedMotion || slideCount < 2) return;
    const timer = window.setInterval(() => setActiveIndex((index) => (index + 1) % slideCount), settings.intervalMs);
    return () => window.clearInterval(timer);
  }, [paused, reducedMotion, settings.autoPlay, settings.intervalMs, slideCount]);

  return (
    <section aria-label={settings.ariaLabel} aria-roledescription="carousel" role="region" className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6" onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onKeyDown={(event) => {
      if (event.key === "ArrowLeft") { event.preventDefault(); navigate(activeIndex - 1); }
      if (event.key === "ArrowRight") { event.preventDefault(); navigate(activeIndex + 1); }
      if (event.key === "Home") { event.preventDefault(); navigate(0); }
      if (event.key === "End") { event.preventDefault(); navigate(slideCount - 1); }
    }}>
      <p className="sr-only" aria-live="polite">第 {activeIndex + 1} 張，共 {slideCount} 張</p>
      <div className="relative overflow-hidden rounded-3xl bg-slate-950 shadow-xl" onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientX ?? null; }} onTouchCancel={() => { touchStart.current = null; }} onTouchEnd={(event) => {
        ...
        <div className={`flex ease-out ${reducedMotion ? "" : "transition-transform duration-500"}`} style={{ transform: `translateX(-${activeIndex * 100}%)` }}>
          {settings.slides.map((slide, index) => {
            ...
            return <div key={slide.id} className="relative min-w-full" aria-hidden={index !== activeIndex} role="group" aria-roledescription="slide" aria-label={`${index + 1} / ${slideCount}`} inert={index !== activeIndex ? true : undefined}>{slide.linkUrl ? <a href={slide.linkUrl} tabIndex={index === activeIndex ? 0 : -1}>{content}</a> : content}</div>;
          })}
        </div>
