...
import { BarChart3, Flame, Gift, PartyPopper, Trophy, Zap } from "lucide-react";
...
export function LiveInteractionStudio({
  ...
  return (
    <section className="mb-6 rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-red-50 p-5 shadow-sm" aria-labelledby="live-interaction-studio-title">
      ...
      {drawRunIds.length > 0 ? (
        <form action={drawAction} className="mt-5 flex flex-wrap items-end gap-3 border-t border-violet-100 pt-4">
          <input type="hidden" name={CSRF_FIELD_NAME} value={csrfToken} />
          <label className="grid min-w-64 gap-1 text-sm font-semibold text-slate-700">
            ...
            <select name="runId" className="h-11 rounded-xl border border-slate-300 bg-white px-3">
              {drawRunIds.map((run) => (
                <option key={run.id} value={run.id}>{run.title}（{run.responseCount} 人）</option>
              ))}
            </select>
          </label>
          ...
          {drawState.message ? (
            <p role={drawState.status === "error" ? "alert" : "status"} className="text-sm font-semibold text-slate-700">
              {drawState.message}
            </p>
          ) : null}
        </form>
