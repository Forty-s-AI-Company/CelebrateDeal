...
export function WorkspaceProjectSwitcher({
  ...
  const [optimisticProjectId, setOptimisticProjectId] = useState<string | null | undefined>();
  const [pending, startTransition] = useTransition();
  const displayProjectId = optimisticProjectId === undefined ? selectedProjectId : optimisticProjectId;
  const current = projects.find((project) => project.id === displayProjectId);

  function choose(projectId: string | null) {
    const previousProjectId = displayProjectId;
    // Reflect the user's scope choice immediately while the server persists it.
    setOptimisticProjectId(projectId);
    setOpen(false);
    startTransition(async () => {
      try {
        await selectProject(projectId);
        setOptimisticProjectId(undefined);
      } catch (error) {
        setOptimisticProjectId(previousProjectId);
        throw error;
      }
    });
  }
  ...
  return (
    <div className="relative mb-3 rounded-xl border border-slate-200 bg-slate-50/80 p-2" data-workspace-switcher>
      <p className="flex items-center gap-2 px-2 pt-1 text-[10px] font-semibold uppercase tracking-wider text-slate-600"><Building2 size={13} aria-hidden="true" />商家 Workspace</p>
      ...
      {open ? <div role="menu" aria-label="切換銷售專案" className="absolute left-2 right-2 top-full z-50 mt-1 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
        <button role="menuitemradio" aria-checked={!displayProjectId} type="button" onClick={() => choose(null)} className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-left text-sm hover:bg-slate-50"><Check size={15} className={!displayProjectId ? "opacity-100" : "opacity-0"} aria-hidden="true" /><span><b className="block">全部專案總覽</b><span className="text-xs text-slate-500">商家彙總，僅供檢視</span></span></button>
        {projects.map((project) => <button key={project.id} role="menuitemradio" aria-checked={project.id === displayProjectId} type="button" onClick={() => choose(project.id)} className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-left text-sm hover:bg-slate-50"><Check size={15} className={project.id === displayProjectId ? "opacity-100" : "opacity-0"} aria-hidden="true" /><span className="min-w-0 flex-1 truncate">{project.name}</span></button>)}
