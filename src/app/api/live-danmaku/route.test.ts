...
describe("live danmaku route", () => {
  beforeEach(() => {
    mocks.auth.mockReset().mockResolvedValue(null);
    mocks.rateLimit.mockReset().mockResolvedValue(null);
  });

  it("拒絕未授權的講師讀寫，並禁止快取", async () => {
    const getResponse = await GET(new Request("https://app.example.test/api/live-danmaku?mode=instructor&liveId=live-1"));
    const postResponse = await POST(new Request("https://app.example.test/api/live-danmaku?mode=instructor", { method: "POST" }));
