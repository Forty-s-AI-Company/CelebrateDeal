...
describe("scripted danmaku route", () => {
  beforeEach(() => {
    mocks.auth.mockReset().mockResolvedValue(null);
    mocks.rateLimit.mockReset().mockResolvedValue(null);
  });

  it("拒絕未授權的腳本讀寫，並禁止快取", async () => {
    const getResponse = await GET(new Request("https://app.example.test/api/live-danmaku/scripted?liveId=live-1"));
    const postResponse = await POST(new Request("https://app.example.test/api/live-danmaku/scripted", { method: "POST" }));
