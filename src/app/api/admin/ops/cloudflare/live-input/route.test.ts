...
function authorizedRequest(payload: Record<string, unknown>) {
  return new Request("https://app.example.test/api/admin/ops/cloudflare/live-input", {
    method: "POST",
    headers: {
...
describe("POST /api/admin/ops/cloudflare/live-input", () => {
  it.each([
  ...
  it("returns only the allowed live-input fields without a plaintext stream key", async () => {
    mocks.createLiveInputMapping.mockResolvedValue({
      video: {
        ...
        liveStreamKey: "test-fixture-plaintext-stream-key",
      },
      liveInput: {
        uid: "live-input-1",
        rtmps: {
