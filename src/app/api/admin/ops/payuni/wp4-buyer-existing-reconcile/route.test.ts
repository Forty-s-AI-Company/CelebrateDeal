...
vi.mock("@/lib/wp4-payuni-buyer-continuation", () => ({ continueWp4BuyerReconcile: mocks.helper }));
import { POST } from "./route";
const secret = "buyer-continuation-test-secret";
...
beforeEach(() => { vi.clearAllMocks(); vi.stubEnv("JOB_SECRET", secret); vi.stubEnv("VERCEL_ENV", "preview"); vi.stubEnv("PAYUNI_ENV", "sandbox"); vi.stubEnv("WP4_SANDBOX_EXECUTOR_ENABLED", "true"); vi.stubEnv("VERCEL_GIT_COMMIT_SHA", source); mocks.helper.mockResolvedValue({ status: "PENDING_RESERVATION_UNAVAILABLE", reconciled: false }); });
...
describe("buyer existing reconcile route", () => {
  it("fails closed for auth, environment, source and body before delegate", async () => { expect((await POST(request())).status).toBe(401); expect((await POST(request({ auth: true, body: "{}" }))).status).toBe(404); vi.stubEnv("PAYUNI_ENV", "production"); expect((await POST(request({ auth: true }))).status).toBe(404); vi.stubEnv("PAYUNI_ENV", "sandbox"); expect((await POST(request({ auth: true, sha: "b".repeat(40) }))).status).toBe(404); expect(mocks.helper).not.toHaveBeenCalled(); });
  it("returns valid result as HTTP 200", async () => { const response = await POST(request({ auth: true })); expect(response.status).toBe(200); expect(mocks.helper).toHaveBeenCalledExactlyOnceWith(mocks.db); });
});
