...
vi.mock("@/lib/wp4-payuni-sandbox-refund-execution", () => ({ executeWp4PayUniSandboxSubscriptionRefund: mocks.execute }));

import { POST } from "./route";

const jobSecret = "test-wp4-subscription-refund-job-secret";
const sourceSha = "a".repeat(40);

function request(options: { authorization?: string; sha?: string; body?: BodyInit } = {}) {
  return new Request("https://app.example.test/api/admin/ops/payuni/wp4-subscription-refund", {
    method: "POST",
    headers: {
...
beforeEach(() => {
  ...
  mocks.execute.mockResolvedValue({ status: "COMPLETED", purpose: "platform_subscription", phase: "remaining", providerWriteAttempted: true });
});

afterEach(() => vi.unstubAllEnvs());

describe("POST /api/admin/ops/payuni/wp4-subscription-refund", () => {
  ...
  it("only runs for the exact Preview Sandbox source and returns a closed projection", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ status: "COMPLETED", purpose: "platform_subscription", phase: "remaining", providerWriteAttempted: true });
  ...
  ])("rejects unsafe %s before refund execution", async (key, value) => {
  ...
  it("rejects a different source without execution", async () => {
    const response = await POST(request({ authorization: `Bearer ${jobSecret}`, sha: "b".repeat(40) }));
    expect(response.status).toBe(404);
    expect(mocks.execute).not.toHaveBeenCalled();
  });

  it("does not expose an internal execution error", async () => {
    mocks.execute.mockRejectedValueOnce(new Error("synthetic-private-detail"));
    const response = await POST(request({ authorization: `Bearer ${jobSecret}` }));
