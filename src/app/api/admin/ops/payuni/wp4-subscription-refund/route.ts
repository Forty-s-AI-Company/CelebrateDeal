...
import { executeWp4PayUniSandboxSubscriptionRefund } from "@/lib/wp4-payuni-sandbox-refund-execution";
...
function responseStatus(status: Awaited<ReturnType<typeof executeWp4PayUniSandboxSubscriptionRefund>>["status"]) {
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeWp4PayUniSandboxSubscriptionRefund(getDb(), sourceCommit);
