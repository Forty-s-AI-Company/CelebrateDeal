...
vi.mock("@/lib/wp4-payuni-sandbox-reconciliation", () => ({
  reconcileWp4PayUniSandboxRefund: mocks.reconcile,
}));

import { POST } from "./route";

const jobSecret = "test-wp4-reconcile-job-secret";
const sourceSha = "a".repeat(40);

function request(options: { authorization?: string; sha?: string; body?: BodyInit } = {}) {
  return new Request("https://app.example.test/api/admin/ops/payuni/wp4-reconcile", {
    method: "POST",
    headers: {
...
describe("POST /api/admin/ops/payuni/wp4-reconcile", () => {
