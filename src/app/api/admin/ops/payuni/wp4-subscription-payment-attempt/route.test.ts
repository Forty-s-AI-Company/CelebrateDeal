...
vi.mock("@/lib/wp4-payuni-sandbox-payment-attempt", () => ({
  reserveWp4PayUniSubscriptionPaymentAttempt: mocks.reserve,
}));
...
function request(options: { authorization?: string; sha?: string; body?: BodyInit } = {}) {
  return new Request("https://preview.example.test/api/admin/ops/payuni/wp4-subscription-payment-attempt", {
    method: "POST",
    headers: {
...
describe("POST /api/admin/ops/payuni/wp4-subscription-payment-attempt", () => {
  ...
  ])("returns only a closed SaaS reservation result %#", async (result, status) => {
