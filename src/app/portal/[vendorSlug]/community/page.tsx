...
export default async function CommunityPage({ params }: { params: Promise<{ vendorSlug: string }> }) {
  ...
  const posts = await db.communityPost.findMany({
    where: { vendorId: session.vendorId }, orderBy: [{ isPinned: "desc" }, { createdAt: "desc" }], take: 50,
    include: { comments: { orderBy: { createdAt: "asc" }, take: 100 }, reactions: { select: { customerKeyHash: true } } },
  });
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950 sm:px-6">
    <div className="mx-auto max-w-4xl">
      ...
      <section className="mt-7 space-y-4" aria-label="社群貼文">{posts.map((post) => <article key={post.id} className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-2 border-t border-slate-200 pt-4">{post.comments.map((comment) => <div key={comment.id} className="rounded-xl bg-slate-50 p-3"><p className="text-sm font-bold">{comment.authorName}</p><p className="mt-1 whitespace-pre-wrap break-words text-sm">{comment.body}</p></div>)}</div>
