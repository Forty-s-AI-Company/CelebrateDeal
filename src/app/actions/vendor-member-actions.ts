...
export async function createVendorMemberAction(formData: FormData) {
  ...
  if (!email || !name || !MEMBER_ROLES.has(role)) redirect("/settings/team?error=member_invalid");
  ...
  if (rateLimited) redirect(`/settings/team?error=${rateLimited.status === 429 ? "member_invitation_rate_limited" : "member_invitation_unavailable"}`);
  ...
  if (existingUser?.platformRole && existingUser.platformRole !== "none") redirect("/settings/team?error=platform_user");
  ...
  if (existingMember?.userId === auth.user.id && role !== "owner") redirect("/settings/team?error=self_role");
  ...
  try {
    ...
    if (error instanceof LastOwnerInvariantError || isDatabaseTransactionConflict(error)) redirect("/settings/team?error=last_owner");
    throw error;
  }
...
export async function resendVendorMemberInvitationAction(formData: FormData) {
  ...
  if (member?.status !== "active" || member.userId === auth.user.id || member.user.platformRole !== "none") redirect("/settings/team?error=member_invitation_resend_invalid");
...
export async function deactivateVendorMemberAction(formData: FormData) {
  ...
  if (!member || member.status !== "active" || member.user.platformRole !== "none") redirect("/settings/team?error=member_not_found");
  if (member.userId === auth.user.id) redirect("/settings/team?error=self_deactivate");
  if (confirmation !== normalizedEmail(member.user.email)) redirect("/settings/team?error=member_confirmation");
    ...
    try {
      return await db.$transaction(async (tx) => {
        ...
        if (member.role === "owner") {
          ...
          if (activeOwnerCount === 0) redirect("/settings/team?error=last_owner");
        }
      ...
      if (member.role === "owner" && isSerializationConflict(error)) redirect("/settings/team?error=last_owner");
      if (isDatabaseTransactionConflict(error)) redirect("/settings/team?error=member_not_found");
      throw error;
    }
  })();
  await writeAuditLog({
