...
export default async function NewLandingPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const goal: Exclude<FunnelGoal, "webinar"> | null = query.goal === "audience" || query.goal === "sell" || query.goal === "custom" ? query.goal : null;
  ...
  return <LandingPageWorkspace initialGoal={goal} initialName={typeof query.name === "string" ? query.name : undefined} initialSlug={typeof query.slug === "string" ? query.slug : undefined} forms={forms} lives={lives.map((live) => ({ ...live, status: live.status as "scheduled" | "live" | "ended", scheduledAt: live.scheduledAt.toISOString(), timezone: vendor.timezone }))} csrfName={CSRF_FIELD_NAME} csrfToken={await getCsrfToken()} />;
}
