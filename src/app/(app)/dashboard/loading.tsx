export default function DashboardLoading() {
  return (
    <main aria-busy="true" aria-labelledby="dashboard-loading-title" className="grid gap-6">
      <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5" aria-hidden="true">
        {Array.from({ length: 10 }, (_, index) => (
          <div key={index} className="h-32 animate-pulse rounded-xl border border-slate-200 bg-slate-100" />
        ))}
      </div>
    </main>
  );
