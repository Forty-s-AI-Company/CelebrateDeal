...
const mocks = vi.hoisted(() => ({ requireVendorManagerContext: vi.fn(), findMany: vi.fn(), groupBy: vi.fn() }));
...
vi.mock("@/components/ui", () => ({
  ...
  ListSummary: ({ items }: { items: Array<{ label: string; value: string | number }> }) => <div>{items.map((item) => <span key={item.label}>{item.label}:{item.value}</span>)}</div>,
...
beforeEach(() => { vi.clearAllMocks(); mocks.requireVendorManagerContext.mockResolvedValue({ auth: { user: { id: "user-1" } }, vendor: { id: "vendor-1", name: "商家" } }); mocks.findMany.mockResolvedValue([{ id: "form-1", name: "活動報名", slug: "summer", isActive: true, _count: { submissions: 2 } }, { id: "form-2", name: "停用表單", slug: "old", isActive: false, _count: { submissions: 0 } }]); mocks.groupBy.mockResolvedValue([{ formId: "form-1", _count: { _all: 1 } }]); });
