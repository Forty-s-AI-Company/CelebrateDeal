...
describe("/interaction-roles/new route", () => {
  it("loads existing vendor roles for the new-role workbench", async () => { const html = renderToStaticMarkup(await NewInteractionRolePage()); expect(mocks.roleFindMany).toHaveBeenCalledWith({ where: { vendorId: "vendor-1" }, orderBy: { createdAt: "desc" } }); expect(html).toContain("新增使用者時"); expect(html).toContain("匯入 10 個官方角色"); expect(html).toContain("role-1"); expect(html).toContain("csrf-token"); });
