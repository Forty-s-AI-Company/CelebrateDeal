...
export default async function ConsultationsPage({ searchParams }: { searchParams: Promise<PageSearchParams> }) {
  ...
  return (
    <>
      ...
      {!scope.isAggregate ? (
        <Link href="/consultations/mobile" className="mb-4 inline-flex min-h-11 items-center rounded-lg bg-slate-950 px-4 text-sm font-bold text-white">
          開啟顧問手機極速工作台
        </Link>
      ) : null}
      {params.updated ? (
      ...
      <Card className="mb-6">
        ...
        {events.length ? (
              ...
              <div key={event.id} className="grid gap-3 rounded-lg border border-border p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge tone={event.isActive ? "green" : "gray"}>{event.isActive ? "開放預約" : "已停用"}</Badge>
                  </div>
                  <p className="mt-1 text-sm text-slate-600">
                    {event.durationMinutes} 分鐘 · 緩衝 {event.bufferMinutes} 分鐘 · {event.dailyLimit ? `每日最多 ${event.dailyLimit} 筆` : "不設每日上限"} · {event.timezone}
                  </p>
          ...
          <p className="text-sm text-slate-600">還沒有諮詢活動。先設定一組開放時段，公開漏斗就能串上預約元件。</p>
        )}
      </Card>
      ...
      <Card>
        ...
        {bookings.length ? (
              ...
              <tbody>
                {bookings.map((booking) => (
                  <tr key={booking.id} className="border-b border-border">
                    <td className="p-2 whitespace-nowrap">{booking.startTime.toLocaleString("zh-TW")}</td>
                    <td className="p-2">{eventNames.get(booking.eventId) ?? "已移除活動"}</td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      {booking.status === "scheduled" && !scope.isAggregate ? (
                        <div className="flex flex-wrap gap-2">
                          <form action={updateConsultationBookingStatusAction}>
                            <CsrfField />
                            <input type="hidden" name="bookingId" value={booking.id} />
                            <input type="hidden" name="status" value="completed" />
                            <button type="submit" className="min-h-11 rounded-md border border-border px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                              完成
                            </button>
                          </form>
                          <form action={updateConsultationBookingStatusAction}>
                          ...
                          <form action={cancelConsultationBookingAction}>
                            <CsrfField />
                            <input type="hidden" name="bookingId" value={booking.id} />
                            <button type="submit" className="min-h-11 rounded-md border border-border px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                              取消預約
          ...
          <p className="text-sm text-slate-600">目前還沒有預約。</p>
        )}
        {bookings.some((booking) => booking.answers) ? (
          <section className="mt-6 border-t border-border pt-4" aria-labelledby="consultation-answers-title">
            <h3 id="consultation-answers-title" className="mb-3 font-semibold text-slate-900">
              行前診斷問卷
            </h3>
            <div className="grid gap-3">
              {bookings
                .filter((booking) => booking.answers)
                .map((booking) => (
                  <details key={`answers-${booking.id}`} className="rounded-md border border-border p-3">
                    <summary className="cursor-pointer text-sm font-medium">
                      {booking.clientName} · {booking.startTime.toLocaleString("zh-TW")}
                    </summary>
