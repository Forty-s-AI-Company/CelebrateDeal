...
/** Match the checkout key generated by the isolated WP4 runner for this deployment. */
function fixedCheckoutKey(sourceSha: string) {
  if (!/^[a-f0-9]{40}$/u.test(sourceSha)) return null;
  const hex = createHash("sha256").update(`celebratedeal-mvp-payuni:${sourceSha}`, "utf8").digest("hex").slice(0, 32);
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-4${hex.slice(13, 16)}-8${hex.slice(17, 20)}-${hex.slice(20)}`;
}

function matchesFixedCommercialTerms(payment: SnapshotPayment, order: SnapshotOrder, sourceSha: string) {
  return payment.status === "paid" && order.status === "paid"
    && order.orderNumber === payment.orderNumber
    && payment.checkoutIdempotencyKey === fixedCheckoutKey(sourceSha)
...
async function readSnapshot(tx: SnapshotDb, sourceSha: string): Promise<BuyerOrderProof> {
  ...
  if (!matchesFixedCommercialTerms(payment, order, sourceSha) || !hasOnlyFixedSyntheticProduct(order)
