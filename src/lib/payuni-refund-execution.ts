...
function hasValidRefundAmounts(input: Pick<PayUniRefundExecutionInput,
  "refundAmountCents" | "gatewayFeeRefundCents" | "platformFeeRefundCents"
>) {
  return Number.isSafeInteger(input.refundAmountCents)
    && input.refundAmountCents > 0
    && Number.isSafeInteger(input.gatewayFeeRefundCents)
    && input.gatewayFeeRefundCents >= 0
    && Number.isSafeInteger(input.platformFeeRefundCents)
    && input.platformFeeRefundCents >= 0;
}
...
export async function executePayUniRefund(input: PayUniRefundExecutionInput): Promise<PayUniRefundExecutionResult> {
  if (!hasValidRefundAmounts(input)) return { disposition: "validation_failed" };
    ...
    for (let attempt = 1; attempt <= MAX_COMPLETION_ATTEMPTS; attempt += 1) {
      try {
        const completed = await input.db.$transaction(async (tx) => {
          ...
          await applyPlatformRefundProjection(tx, transaction, refundOccurredAt);
