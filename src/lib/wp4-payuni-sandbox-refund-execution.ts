...
function matchesPurpose(candidate: Candidate, sourceCommit: string, purpose: Wp4RefundPurpose) {
 ...
 * transaction, amount, phase, or purpose. An ambiguous fixture set fails
 * closed before the provider refund function is reached.
 */
async function executeFixedWp4PayUniSandboxRefund(
  ...
  if (result.disposition === "provider_request_rejected") {
    return {
      ...
      purpose,
      phase,
      providerWriteAttempted: result.category !== "request_contract",
    };
  }
  if (result.disposition === "validation_failed") {
    return { status: "REFUND_NOT_ELIGIBLE", purpose, phase, providerWriteAttempted: false };
  }
  return { status: "RECONCILIATION_REQUIRED", purpose, phase, providerWriteAttempted: true };
}

/** Executes only the buyer-order refund. SaaS rows cannot satisfy this wrapper. */
...
export async function executeWp4PayUniSandboxSubscriptionRefund(
  db: RefundExecutionDb,
  sourceCommit: string,
  now = new Date(),
): Promise<Wp4PayUniSandboxRefundExecutionResult> {
  return executeFixedWp4PayUniSandboxRefund(db, sourceCommit, "platform_subscription", now);
}
