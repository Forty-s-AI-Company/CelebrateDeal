...
describe("commerce orders database service", () => {
  ...
  it("creates a second immutable item from a server-authorized same-tenant order bump", async () => {
    const tx = transaction();
    ...
    expect(tx.commerceOrder.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ subtotalAmountCents: 1_500, totalAmountCents: 1_500 }),
    }));
  ...
  it("rejects an order bump that cannot be re-read inside the same tenant transaction", async () => {
    const tx = transaction();
