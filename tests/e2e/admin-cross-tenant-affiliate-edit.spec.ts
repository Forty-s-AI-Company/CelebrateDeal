...
test("admin cannot open another vendor affiliate edit route through direct URL navigation", async ({ page }) => {
  ...
  try {
    ...
    const ownPath = `/affiliates/${ownAffiliate.id}/edit`; const ownResponse = await page.goto(ownPath); expect(ownResponse?.status()).toBe(200); await expect(page).toHaveURL(new RegExp(`${ownPath}$`)); await expect(page.getByRole("heading", { name: "編輯聯盟夥伴" })).toBeVisible(); await expect(page.getByLabel("夥伴名稱")).toHaveValue(ownAffiliate.name); await expect(page.getByRole("textbox", { name: "專屬推廣碼", exact: true })).toHaveValue(ownAffiliate.code); await expect(page.getByLabel("來源渠道")).toHaveValue(ownAffiliate.source ?? ""); await expect(page.getByLabel("聯絡 Email")).toHaveValue(ownAffiliate.contactEmail ?? ""); await expect(page.getByLabel("佣金比例")).toHaveValue(String(ownAffiliate.commissionRateBps)); await expect(page.getByText(ownPathCanary, { exact: true })).toBeVisible();
    ...
    for (const heading of ["編輯聯盟夥伴", "最近來源事件"]) await expect(page.getByRole("heading", { name: heading })).toHaveCount(0); for (const label of ["夥伴名稱", "來源渠道", "聯絡 Email", "佣金比例"]) await expect(page.getByLabel(label)).toHaveCount(0); await expect(page.getByRole("textbox", { name: "專屬推廣碼", exact: true })).toHaveCount(0); for (const value of foreignCanaries) await expect(page.getByText(value, { exact: true })).toHaveCount(0);
