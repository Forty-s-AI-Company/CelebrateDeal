...
test("active accountant is denied same-vendor brand settings through direct URL navigation", async ({ page }) => {
  ...
  try {
    ...
    const path = "/settings/brand"; const raw = page.waitForResponse((response) => new URL(response.url()).pathname === path && response.status() === 307); const final = await page.goto(path); const response = await raw;
    expect(final?.status()).toBe(200); expect(new URL(response.headers().location ?? "", "http://127.0.0.1").pathname + new URL(response.headers().location ?? "", "http://127.0.0.1").search).toBe("/dashboard?error=insufficient_role"); await expect(page).toHaveURL(/\/dashboard\?error=insufficient_role$/);
