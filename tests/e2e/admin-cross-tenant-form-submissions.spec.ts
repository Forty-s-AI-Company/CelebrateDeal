...
test("admin cannot open another vendor form submissions through direct URL navigation", async ({ page }) => {
  ...
  const snapshot = async () => Promise.all([
    db.registrationForm.findUniqueOrThrow({ where: { id: ownForm.id } }),
    db.registrationForm.findUniqueOrThrow({ where: { id: foreignForm.id } }),
    db.formSubmission.findMany({ where: { formId: ownForm.id }, orderBy: { id: "asc" } }),
    db.formSubmission.findMany({ where: { formId: foreignForm.id }, orderBy: { id: "asc" } }),
    db.registrationForm.count({ where: { id: ownForm.id } }),
    db.registrationForm.count({ where: { id: foreignForm.id } }),
    db.formSubmission.count({ where: { formId: ownForm.id } }),
    db.formSubmission.count({ where: { formId: foreignForm.id } }),
  ]);
  try {
    ...
    const foreignPath = `/forms/${foreignForm.id}/submissions`;
    const foreignDataCanaries = [foreignForm.name, foreignForm.slug, foreignCanary, `${foreignCanary}@example.test`, "0900000002"];
    await navigateAndAssertDirectUrlGuard({
      page,
      path: foreignPath,
      routeIdentityCanaries: [foreignForm.id],
    ...
    await expect(page.getByRole("heading", { name: `${foreignForm.name} 名單` })).toHaveCount(0); await expect(page.getByRole("table")).toHaveCount(0); await expect(page.getByText(foreignCanary)).toHaveCount(0); await expect(page.getByText(`${foreignCanary}@example.test`)).toHaveCount(0); await expect(page.getByText("0900000002")).toHaveCount(0);
