...
test("admin forms index does not expose another vendor forms or submission counts", async ({ page }) => {
  ...
  try {
    await page.goto("/login", { waitUntil: "load" }); await expect(page).toHaveURL(/\/login$/); await expect(page.locator("main").first()).toBeVisible(); await page.getByLabel("Email").fill(user.email); await page.getByLabel("密碼").fill(password); await page.getByRole("button", { name: "登入" }).click(); await expect(page).toHaveURL(/\/dashboard$/); await page.waitForLoadState("load"); await expect(page.locator("main").first()).toBeVisible();
    const response = await page.goto("/forms", { waitUntil: "load" }); expect(response?.status()).toBe(200); await expect(page).toHaveURL(/\/forms$/); await expect(page.locator("main").first()).toBeVisible(); await expect(page.getByRole("heading", { name: "報名表管理" })).toBeVisible(); await expect(page.getByText(ownForm.name, { exact: true })).toBeVisible(); await expect(page.getByText(`/form/${ownForm.slug}`, { exact: true })).toBeVisible(); await expect(page.locator(`a[href="/forms/${ownForm.id}/edit"]`)).toHaveCount(1); await expect(page.locator(`a[href="/forms/${ownForm.id}/submissions"]`)).toHaveCount(1); await expect(page.getByText("0 已驗證 / 1 待驗證", { exact: true })).toHaveCount(1);
