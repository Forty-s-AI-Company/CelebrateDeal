...
test("Webinar owner saves settings, resolves publish validation and opens every public step", async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveURL(/goal=webinar/u);
  await expect(page.getByLabel("Funnel steps")).toContainText("Webinar 播放頁");
  const gallery = page.getByLabel("Funnel steps").getByText("模板 Gallery", { exact: true });
  await expect(gallery).toBeVisible();
  const previewButton = page.getByRole("button", { name: "完整預覽", exact: true }).first();
  await previewButton.focus();
  await expect(previewButton).toBeFocused();
  await previewButton.click();
  const templateDialog = page.getByRole("dialog", { name: /專注播放會場/u });
  ...
  await page.getByLabel("步驟名稱", { exact: true }).fill("TEST ONLY Webinar 報名頁");
  await page.getByRole("button", { name: "流程 Undo", exact: true }).click();
  await expect(page.getByLabel("步驟名稱", { exact: true })).toHaveValue("Webinar 報名頁");
  await page.getByRole("button", { name: "流程 Redo", exact: true }).click();
  await expect(page.getByLabel("步驟名稱", { exact: true })).toHaveValue("TEST ONLY Webinar 報名頁");
  await page.getByLabel("步驟 URL Path", { exact: true }).fill("registration-preview");
  await expect(page.getByLabel("步驟 URL Path", { exact: true })).toHaveValue("registration-preview");
  await page.getByRole("button", { name: "流程 Undo", exact: true }).click();
  await expect(page.getByLabel("步驟 URL Path", { exact: true })).toHaveValue("registration");
  ...
  await expect(headline).toHaveText(editedHeadline);
  const steps = page.getByLabel("Funnel steps");
  await steps.getByRole("button", { name: /^Webinar 感謝頁\s*\/thank-you$/u }).click();
  await expect(headline).not.toHaveText(editedHeadline);
  await steps.getByRole("button", { name: /^TEST ONLY Webinar 報名頁\s*\/registration$/u }).click();
  await expect(headline).toHaveText(editedHeadline);
  ...
  for (const step of ["registration", "thank-you", "broadcast", "inactive"]) {
