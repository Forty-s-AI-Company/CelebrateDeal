...
test("active accountant is denied interaction-script creation before tenant data, CSRF, or form render", async ({
  ...
  try {
    ...
    await page.route("**/interaction-scripts/new", async (route) => {
    ...
    expect(intercepted.current?.body).not.toContain("新增互動腳本");
    expect(intercepted.current?.body).not.toContain("常見留言組範本");
