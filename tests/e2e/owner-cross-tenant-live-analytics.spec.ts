...
test("owner cannot open another vendor live analytics route through direct URL navigation", async ({ page }) => {
  ...
  try {
    ...
    for (const heading of ["轉換漏斗", "最近事件", "聯盟來源"]) await expect(page.getByRole("heading", { name: heading, exact: true })).toBeVisible();
    ...
    for (const heading of [`${foreignLive.title} 分析`, "最近事件", "聯盟來源"]) await expect(page.getByRole("heading", { name: heading, exact: true })).toHaveCount(0);
