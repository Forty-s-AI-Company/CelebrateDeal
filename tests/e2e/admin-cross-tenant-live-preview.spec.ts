...
test("admin cannot open another vendor live preview through direct URL navigation", async ({ page }) => {
  ...
  try {
    ...
    const foreignPath = `/lives/${foreignLive.id}/preview`; const foreignResponse = await page.goto(foreignPath); expect(foreignResponse?.status()).toBe(404); await expect(page).toHaveURL(new RegExp(`${foreignPath}$`)); await expect(page.getByRole("heading", { name: "直播預覽" })).toHaveCount(0); await expect(page.getByRole("link", { name: "開啟公開頁" })).toHaveCount(0);
    const foreignCanaries = [foreignLive.title, foreignLive.description ?? "", foreignLive.slug, foreignLive.cloudflareLiveInputUid ?? ""]; for (const value of foreignCanaries) await expect(page.getByText(value, { exact: true })).toHaveCount(0); const content = await page.content(); for (const value of foreignCanaries) expect(content).not.toContain(value);
