...
test("active accountant is denied interaction-role creation before role data, CSRF, or controls render", async ({
  ...
  try {
    ...
    await page.route("**/interaction-roles/new", async (route) => {
    ...
    expect(intercepted.current?.body).not.toContain("匯入 10 個官方角色");
    expect(intercepted.current?.body).not.toContain("使用者清單");
