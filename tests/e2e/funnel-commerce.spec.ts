...
test("商品綁定、可信價格與兩步驟結帳（付款 transport mock，非 sandbox）", async ({ page, baseURL }, testInfo) => {
  ...
  const orderBumpSelect = page.getByRole("combobox", { name: /^加購商品（選填）/u });
  await orderBumpSelect.selectOption(fixture.bump.id);
  await expect(orderBumpSelect).toHaveValue(fixture.bump.id);
  const formModeSelect = page.getByRole("combobox", { name: /^結帳版面/u });
  await formModeSelect.selectOption("two_step");
  await expect(formModeSelect).toHaveValue("two_step");
  const agreement = "TEST ONLY 我同意此商品交付條款";
  const agreementInput = page.getByRole("textbox", { name: "額外同意條款（選填）", exact: true });
  await agreementInput.fill(agreement);
  await expect(agreementInput).toHaveValue(agreement);
  await expect(page.getByRole("status").filter({ hasText: "有尚未儲存的變更" })).toBeVisible();
  ...
  await page.route("**/api/payments/checkout", async route => {
    ...
    validCheckoutShape = body.productId === fixture.product.id && body.agreementAccepted === true
      ...
      && Number.isInteger(reference.expectedOrderBumpRevision)
