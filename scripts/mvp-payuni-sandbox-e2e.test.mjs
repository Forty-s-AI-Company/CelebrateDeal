...
test("submits the PayUni form at most once and fails closed when confirmation is ambiguous", async () => {
  ...
  const page = {
    ...
    getByRole(_role, options) {
      if (options.name === "確認送出") return { async click() { submitClicks += 1; } };
      if (options.name === "確定") return confirmation;
      throw new Error("unexpected role");
    },
    async waitForURL() { throw new Error("must not wait after ambiguous confirmation"); },
  };
  const browser = {
    async newContext() {
      return {
        ...
        request: {
          async post(url, options) {
            assert.equal(url, "https://sandbox-api.payuni.com.tw/api/upp");
  ...
  const result = await defaultBrowserSubmit({
    previewHost,
    supportCookie: "celebrate_support_wp4=opaque",
    ...
    playwright: {
      chromium: { async launch(options) { launchOptions = options; return browser; } },
      errors: { TimeoutError },
    },
  });
  ...
  assert.equal(Object.hasOwn(launchOptions.env, "JOB_SECRET"), false);
  assert.equal(Object.hasOwn(launchOptions.env, "GITHUB_TOKEN"), false);
...
test("rejects the retired six-entity fixture response instead of broadening its bound", async () => {
  const receipt = await runMvpPayUniSandboxE2E(validInput, {
    request: async () => response(200, { ready: true, createdCount: 6, reusedCount: 0 }),
  });
...
test("final validation failure preserves already executed payment and refund counters", async () => {
  ...
  // Simulate a schema defect after the entire mocked operation has completed.
