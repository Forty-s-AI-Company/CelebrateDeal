...
const focusedWorkPackage = focusBuyerDelivery ? "G7-48B" : focusProductDelivery ? "G7-48A" : focusBuyerOrders ? "G7-47" : focusOnboarding ? "G7-49" : focusStreamQuota ? "G7-50" : focusStreamRetry ? "G7-51" : focusCheckoutRecovery ? "G7-57" : focusMessageTemplateDraft ? "G7-58" : focusInteractionRole ? "G7-52" : focusLiveStudioStarter ? "G7-46" : "G7-45";
...
export async function main() {
  ...
  const focused = focusDelivery || focusBuyerOrders || focusOnboarding || focusStreamQuota || focusStreamRetry || focusCheckoutRecovery || focusMessageTemplateDraft || focusInteractionRole || focusLiveStudio;
  ...
  try {
    if ([focusDelivery, focusBuyerOrders, focusOnboarding, focusStreamQuota, focusStreamRetry, focusCheckoutRecovery, focusMessageTemplateDraft, focusInteractionRole, focusLiveStudio].filter(Boolean).length > 1 || !tempNamePattern.test(name) || !containerNamePattern.test(name) || !schemaPattern.test(schema) || migrations.length === 0) throw new Error("runner-contract-invalid");
