...
const work = fs.mkdtempSync(path.join(os.tmpdir(), 'celebratedeal-operations-qa-'));
let app = path.join(work, 'app');
const name = `celebratedeal-operations-qa-${id}`;
const evidence = path.join(root, 'docs/ai-team/evidence/funnel-operations-20260917');
...
function sourceDigest(base) {
  ...
  const visit = (relative) => {
    ...
    if (fs.statSync(absolute).isDirectory()) {
      for (const item of fs.readdirSync(absolute).sort()) if (!item.startsWith('.env') && !/\.test\.[jt]sx?$/u.test(item)) visit(path.join(relative, item));
    } else { digest.update(relative); digest.update(fs.readFileSync(absolute)); }
  };
  for (const item of ['src', 'public', ...reusableFiles]) visit(item);
  // Build reuse also binds the generated client and database contract.
  const schema = fs.readFileSync(path.join(base, 'prisma/schema.prisma'), 'utf8').replace(/\r\n/gu, '\n').replace(/^\s*output\s*=\s*"\.\.\/node_modules\/\.prisma\/client"\n/gmu, '');
...
if (process.argv.includes('--reuse-build') || process.argv.includes('--refresh-build')) {
  ...
  if (path.dirname(previousRoot) !== path.resolve(os.tmpdir()) || !path.basename(previousRoot).startsWith('celebratedeal-operations-qa-') || previous.sourceEnvRead !== false || (process.argv.includes('--reuse-build') && previous.build !== 'PASS')) throw new Error('Unverified build mirror');
...
const receipt = { browserScope: process.argv.includes('--transport-diagnostic') ? 'minimal transport diagnostic only' : 'complete funnel operations journey', status: 'RUNNING', migrations: 'NOT_RUN', tests: 'NOT_RUN', typecheck: 'NOT_RUN', build: 'NOT_RUN', browser: 'NOT_RUN', cleanup: 'NOT_RUN', sourceEnvRead: false, paymentEvidence: 'no payment or provider interaction; synthetic funnel operations only', externalAssets: 'optional remote Inter stylesheet replaced with inert local CSS; all other external browser requests blocked', target: 'new disposable loopback PostgreSQL', migrationCount: 0 };
...
function must(result, phase) {
  if (result.status !== 0) {
    if (Object.hasOwn(receipt, phase)) receipt[phase] = 'FAIL';
    ...
    const diagnostics = `${result.stdout ?? ''}\n${result.stderr ?? ''}`.split(/\r?\n/u)
      ...
      .map(line => line.replace(/\u001b\[[0-9;]*m/gu, '').slice(0, 250)).slice(0, 40);
...
let created = false;
try {
  // Only remove stale individual source files inside this runner-owned mirror.
  // cpSync otherwise retains deleted route files, invalidating an exact build.
  const mirrorRoot = path.resolve(app);
  function removeStale(relative) {
    const target = path.resolve(app, relative);
    if (!target.startsWith(mirrorRoot + path.sep)) throw new Error('Mirror path escaped');
    if (!fs.existsSync(target)) return;
    if (fs.lstatSync(target).isSymbolicLink()) throw new Error('Source mirror contains a link');
    if (fs.statSync(target).isDirectory()) {
      for (const entry of fs.readdirSync(target)) removeStale(path.join(relative, entry));
    } else if (!fs.existsSync(path.join(root, relative))) fs.unlinkSync(target);
  }
  for (const dir of ['src', 'public', 'tests']) removeStale(dir);
  for (const dir of ['src', 'public', 'tests']) fs.cpSync(path.join(root, dir), path.join(app, dir), { recursive: true, filter: (p) => !path.basename(p).startsWith('.env') });
  ...
  receipt.sourceFingerprintPolicy = 'production sources exclude colocated test files; tests separately fingerprinted';
  const testDigest = crypto.createHash('sha256');
  const digestTests = (relative) => {
    const absolute = path.join(app, relative);
    if (fs.statSync(absolute).isDirectory()) {
      for (const item of fs.readdirSync(absolute).sort()) if (!item.startsWith('.env')) digestTests(path.join(relative, item));
    } else if (relative.startsWith('tests') || /\.test\.[jt]sx?$/u.test(relative)) { testDigest.update(relative); testDigest.update(fs.readFileSync(absolute)); }
  };
  digestTests('src'); digestTests('tests');
  receipt.testSourcesSha256 = testDigest.digest('hex');
  receipt.browserTestSha256 = crypto.createHash('sha256').update(fs.readFileSync(path.join(app, process.argv.includes('--transport-diagnostic') ? 'tests/e2e/funnel-operations-transport.spec.ts' : 'tests/e2e/funnel-operations.spec.ts'))).digest('hex');
  const start = run('docker', ['run', '-d', '--pull=never', '--name', name, '--label', `celebratedeal.operations-qa=${id}`, '-e', 'POSTGRES_PASSWORD=postgres', '-e', 'POSTGRES_DB=celebratedeal_test', '--tmpfs', '/var/lib/postgresql/data', '-p', '127.0.0.1::5432', 'postgres:16-alpine']);
  ...
  if (!fs.existsSync(generatedSchema) || !/\boperations\s+Json\?/u.test(fs.readFileSync(generatedSchema, 'utf8'))) {
    throw new Error('isolated Prisma client schema is stale');
  }
  Object.assign(env, { FUNNEL_OPERATIONS_DISPOSABLE_QA: '1', E2E_TEST_MODE: 'true', E2E_BASE_URL: 'http://127.0.0.1:31149', NEXT_PUBLIC_APP_URL: 'http://127.0.0.1:31149', PAYMENT_PROVIDER: 'demo', RATE_LIMIT_PROVIDER: 'memory', CSRF_SECRET: 'celebratedeal-local-playwright-csrf-secret-v1', CRON_SECRET: 'celebratedeal-local-playwright-cron-secret-v1', JOB_SECRET: 'e2e-job-secret-at-least-16-chars', LIVE_CHAT_INGRESS_SECRET: 'celebratedeal-local-playwright-live-chat-ingress-secret-v1', BANK_ACCOUNT_KEYRING_JSON: JSON.stringify({ activeKeyId: 'playwright', keys: { playwright: Buffer.alloc(32, 7).toString('base64url') } }), G7_COMMERCE_LOOPBACK_TLS_BRIDGE: '1', SMOKE_TEST_EMAIL: 'operations-qa@example.test', FUNNEL_OPERATIONS_QA_SCREENSHOT_DIR: evidence });
  const next = path.join(root, 'node_modules/next/dist/bin/next');
  const tests = run(process.execPath, [path.join(root, 'node_modules/vitest/vitest.mjs'), 'run',
    'src/lib/funnel-operations.test.ts', 'src/lib/funnel-operations-service.test.ts', 'src/lib/funnel-runtime.test.ts', 'src/lib/funnel-runtime-contract.test.ts', 'src/app/api/form-submissions/route.test.ts', 'src/app/api/form-submissions/route.db.test.ts', 'src/app/lp/[slug]/page.test.tsx', 'src/app/(app)/landing-pages/[id]/operations/page.test.tsx',
    'src/lib/funnel-automation-service.test.ts', 'src/app/actions/funnel-automation-actions.test.ts', 'src/components/landing-pages/funnel-automation-settings.test.tsx', 'src/lib/automation-engine.test.ts',
    '--reporter=json', '--outputFile=operations-tests.json'], app, { NODE_ENV: 'test', E2E_TEST_MODE: 'false' });
  receipt.tests = tests.status === 0 ? 'PASS' : 'FAIL';
  const reportPath = path.join(app, 'operations-tests.json');
  ...
  const config = `import { defineConfig } from '@playwright/test';\nexport default defineConfig({testDir:'./tests/e2e',testMatch:'${process.argv.includes('--transport-diagnostic') ? 'funnel-operations-transport.spec.ts' : 'funnel-operations.spec.ts'}',timeout:300000,expect:{timeout:20000},workers:1,retries:0,reporter:'list',use:{baseURL:'http://127.0.0.1:31149',trace:'off',launchOptions:{executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'}},webServer:{command:${JSON.stringify(`"${process.execPath}" "${next}" start --hostname 127.0.0.1 --port 31149`)},url:'http://127.0.0.1:31149/login',reuseExistingServer:false,timeout:120000,stdout:'pipe',stderr:'pipe',env:{NODE_ENV:'production'}}});`;
  fs.writeFileSync(path.join(app, 'playwright.operations.config.ts'), config);
  const browser = run(process.execPath, [path.join(root, 'node_modules/@playwright/test/cli.js'), 'test', '--config', 'playwright.operations.config.ts']);
  receipt.browser = browser.status === 0 ? 'PASS' : 'FAIL';
  must(browser, 'browser'); process.stdout.write('Browser journey PASS (synthetic operations only)\n');
  receipt.browser = 'PASS';
  if (sourceDigest(root) !== receipt.sourceSha256) throw new Error('Workspace source changed during verification; rebuild required');
  receipt.status = 'PASS';
} catch (error) { receipt.status = 'FAIL'; receipt.failure = error.message; process.exitCode = 1; }
finally {
  if (created) {
    const identity = run('docker', ['inspect', '--format', '{{index .Config.Labels "celebratedeal.operations-qa"}}', name]);
