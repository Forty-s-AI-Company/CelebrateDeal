...
try {
  ...
  await viewer.screenshot({ path: path.join(out, "presenter-viewer-mobile.png"), fullPage: true });
  await instructor.screenshot({ path: path.join(out, "presenter-studio.png"), fullPage: true });
  ...
  receipt.status = "PASS";
} catch (error) { receipt.failure = error.message; await instructor?.screenshot({ path: path.join(out, "presenter-browser-failure.png"), fullPage: true }).catch(() => {}); }
finally { await browser?.close(); media.kill(); await new Promise(resolve => server.close(resolve)); await fs.writeFile(path.join(out, "presenter-browser-evidence.json"), JSON.stringify(receipt, null, 2)); }
