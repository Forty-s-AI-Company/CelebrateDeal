...
if (process.argv.includes('--reuse-build') || process.argv.includes('--refresh-build')) {
  ...
  if (path.dirname(previousRoot) !== path.resolve(os.tmpdir()) || !path.basename(previousRoot).startsWith('celebratedeal-landing-qa-') || previous.build !== 'PASS') throw new Error('Unverified build mirror');
...
try {
  ...
  const start = run('docker', ['run', '-d', '--pull=never', '--name', name, '--label', `celebratedeal.landing-qa=${id}`, '-e', 'POSTGRES_PASSWORD=postgres', '-e', 'POSTGRES_DB=celebratedeal_test', '--tmpfs', '/var/lib/postgresql/data', '-p', '127.0.0.1::5432', 'postgres:16-alpine']);
  ...
  Object.assign(env, { E2E_TEST_MODE: 'true', E2E_BASE_URL: 'http://127.0.0.1:31137', NEXT_PUBLIC_APP_URL: 'http://127.0.0.1:31137', PAYMENT_PROVIDER: 'demo', RATE_LIMIT_PROVIDER: 'memory', CSRF_SECRET: 'celebratedeal-local-playwright-csrf-secret-v1', CRON_SECRET: 'celebratedeal-local-playwright-cron-secret-v1', JOB_SECRET: 'e2e-job-secret-at-least-16-chars', LIVE_CHAT_INGRESS_SECRET: 'celebratedeal-local-playwright-live-chat-ingress-secret-v1', BANK_ACCOUNT_KEYRING_JSON: JSON.stringify({ activeKeyId: 'playwright', keys: { playwright: Buffer.alloc(32, 7).toString('base64url') } }), G7_COMMERCE_LOOPBACK_TLS_BRIDGE: '1', SMOKE_TEST_EMAIL: 'landing-qa@example.test', LANDING_QA_SCREENSHOT_DIR: evidence });
  ...
  if (created) {
    const identity = run('docker', ['inspect', '--format', '{{index .Config.Labels "celebratedeal.landing-qa"}}', name]);
