...
export function finalizeMvpPayUniReceipt(receipt) {
  // Validation failure cannot erase a payment/refund that already occurred.
  // Keep the counters so the separate validator rejects the invalid evidence.
...
export async function defaultBrowserSubmit(input, dependencies = {}) {
  ...
  const browser = await chromium.launch({
    headless: true,
    // Do not inherit the runner's secrets. These static values are sufficient
    // for Chromium and intentionally do not read or enumerate process.env.
  ...
  try {
    ...
    try {
      ...
      // A confirmation dialog after the one permitted submission does not
      // establish whether PayUni accepted, deferred, or rejected that submit.
      // Do not click through it or issue another submission: either could turn
      // the fixed one-payment budget into a duplicate provider-side charge.
