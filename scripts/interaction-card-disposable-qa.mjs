...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), "interaction-card-"));
const marker = `interaction-card:${randomUUID()}`;
...
const evidencePath = path.join(root, "docs/live-feature-handoffs/interaction-card-db-evidence.json");
...
try {
  ...
  const result = run(process.execPath, [path.join(root, "node_modules/vitest/vitest.mjs"), "run", "src/lib/interaction-card.db.test.ts", "--reporter=json", "--outputFile", report], env);
  ...
  if (result.status !== 0 || receipt.tests.length !== 5 || receipt.tests.some(t => t.status !== "passed")) throw new Error("TEST_FAILED");
  receipt.status = "PASS";
  ...
  if (path.dirname(temp) === path.resolve(os.tmpdir()) && path.basename(temp).startsWith("interaction-card-") && fs.readFileSync(path.join(temp, ".marker"), "utf8") === marker) fs.rmSync(temp, { recursive: true });
  if (receipt.cleanup !== "PASS") receipt.status = "FAIL";
  fs.writeFileSync(path.join(root, "docs/live-feature-handoffs/interaction-card-db-evidence.json"), JSON.stringify(receipt, null, 2));
}
