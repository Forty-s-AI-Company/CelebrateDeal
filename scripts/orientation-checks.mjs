...
const temp = path.join(root, "tmp/orientation-checks");
...
const phases = {};
const previousAttempts = fs.existsSync("docs/live-feature-handoffs/orientation-checks.json") ? [JSON.parse(fs.readFileSync("docs/live-feature-handoffs/orientation-checks.json", "utf8"))] : [];
...
const files = ["src/components/evergreen-preview-player.tsx", "src/app/(app)/lives/[id]/preview/page.tsx", "scripts/orientation-broadcast-qa.mjs", "scripts/orientation-disposable-qa.mjs", "scripts/orientation-landscape-broadcast-qa.mjs", "scripts/orientation-checks.mjs", "scripts/mobile-viewing-browser-qa.mjs", "src/app/actions.ts", "src/lib/live-studio-draft.ts", "src/lib/live-studio-draft-client.ts", "src/components/live-stepper-form.tsx", "src/components/live-viewing-shell.tsx", "src/app/(app)/lives/[id]/edit/page.tsx", "src/app/(viewer)/live/[slug]/page.tsx", "next.config.ts", "src/lib/presenter-layout.ts", "src/lib/presenter-media.ts", "src/lib/live-media-client.ts", "src/lib/live-media-provider.ts", "src/lib/live-media-cleanup.ts", "src/lib/live-playback-source.ts", "src/lib/live-video-readiness.ts", "src/components/presenter-studio.tsx", "src/components/live-media-receiver.tsx", "src/components/live-playback.tsx", "src/app/api/live-media/route.ts", "src/app/api/live-presenter/route.ts", "src/app/api/cron/live-media/route.ts", "src/app/api/live-admission/route.ts", "src/app/(app)/lives/[id]/presenter/page.tsx", "src/app/(app)/lives/page.tsx", "scripts/presenter-checks.mjs", "scripts/presenter-browser-qa.mjs", "scripts/presenter-disposable-qa.mjs", "src/lib/presenter-layout.db.test.ts"];
const tests = ["src/app/actions.test.ts", "src/lib/live-studio-draft.test.ts", "src/lib/live-studio-draft-client.test.ts", "src/components/live-stepper-form.test.tsx", "src/app/(app)/lives/[id]/edit/page.test.tsx", "src/app/(viewer)/live/[slug]/page.test.tsx", "src/lib/presenter-headers.test.ts", "src/lib/presenter-layout.test.ts", "src/lib/presenter-media.test.ts", "src/lib/live-media-client.test.ts", "src/lib/live-media-provider.test.ts", "src/lib/live-playback-source.test.ts", "src/lib/live-video-readiness.test.ts", "src/components/live-playback.test.tsx", "src/app/api/live-media/route.test.ts", "src/app/api/live-presenter/route.test.ts", "src/app/api/cron/live-media/route.test.ts", "src/app/api/live-admission/route.test.ts"];
phases.lint = run(["node_modules/eslint/bin/eslint.js", ...files, ...tests]);
phases.unit = run(["node_modules/vitest/vitest.mjs", "run", ...tests, "--reporter=json", "--outputFile", path.join(temp, "unit.json")]);
const unitReport = JSON.parse(fs.readFileSync(path.join(temp, "unit.json"), "utf8"));
fs.writeFileSync("docs/live-feature-handoffs/orientation-checks.json", JSON.stringify({ boundary: "isolated local generate/typecheck/lint/unit; no external services", tests: { total: unitReport.numTotalTests, passed: unitReport.numPassedTests, failed: unitReport.numFailedTests }, previousAttempts, generation: currentClient ? "verified existing client schema matches" : "generate command", phases }, null, 2));
