...
const temp = path.join(root, 'tmp/scripted-roles-checks');
...
const files = ['scripts/scripted-roles-browser-qa.mjs', 'src/components/live-chat-panel.tsx', 'scripts/scripted-roles-disposable-qa.mjs', 'src/lib/scripted-roles.ts', 'src/lib/scripted-roles-contract.ts', 'src/lib/scripted-roles.test.ts', 'src/lib/scripted-roles.db.test.ts', 'src/components/scripted-roles-control.tsx', 'src/components/interaction-script-form.tsx', 'src/app/api/live-danmaku/scripted/route.ts', 'src/lib/live-danmaku.ts', 'src/lib/live-danmaku-contract.ts', 'src/lib/live-danmaku.test.ts', 'src/lib/live-danmaku.db.test.ts', 'src/app/api/live-danmaku/route.ts', 'src/components/live-danmaku.tsx', 'src/lib/interaction-card.ts', 'src/components/live-interaction-card.tsx', 'src/components/live-playback.tsx', 'src/components/live-interaction-studio.tsx', 'src/components/presenter-studio.tsx', 'scripts/scripted-roles-checks.mjs', 'scripts/danmaku-browser-qa.mjs', 'scripts/danmaku-disposable-qa.mjs'];
phases.lint = run(['node_modules/eslint/bin/eslint.js', ...files, '--max-warnings=0']);
phases.unit = run(['node_modules/vitest/vitest.mjs', 'run', 'src/components/live-chat-panel.test.tsx', 'src/lib/scripted-roles.test.ts', 'src/components/interaction-script-form.test.tsx', 'src/lib/live-danmaku.test.ts', 'src/lib/interaction-card.test.ts', 'src/lib/interaction-card-timeline.test.ts', 'src/components/live-playback.test.tsx', 'src/app/api/live-interactions/route.test.ts', 'src/app/(viewer)/live/[slug]/page.test.tsx', '--reporter=json', '--outputFile', path.join(temp, 'unit.json')]);
const report = JSON.parse(fs.readFileSync(path.join(temp, 'unit.json'), 'utf8'));
const evidence = path.join(root, 'docs/live-feature-handoffs/scripted-roles-checks.json');
