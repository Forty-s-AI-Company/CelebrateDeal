...
BEGIN
  IF EXISTS (
    ...
    JOIN "RefundRecord" refund
      ON refund."vendorId" = settlement."vendorId"
     AND refund."monthKey" = settlement."monthKey"
     AND refund."status" = 'processed'
    WHERE settlement."lockedAt" IS NOT NULL
  ) THEN
    RAISE EXCEPTION 'Locked legacy settlement periods containing refunds require signed-ledger reconciliation before deployment';
  END IF;
END $$;
...
ALTER TABLE "PaymentTransaction"
  ADD COLUMN "refundedGatewayFeeCents" INTEGER NOT NULL DEFAULT 0,
