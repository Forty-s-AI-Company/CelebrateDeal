...
if (!schema || !/^g7_13_[a-f0-9]{16}$/u.test(schema)) {
...
if (new URL(databaseUrl).searchParams.get("schema") !== schema || new URL(directUrl).searchParams.get("schema") !== schema) {
  throw new Error("[g7_analytics_authenticity_db] database URLs do not match the disposable schema marker");
}
...
export default defineConfig({
  ...
  test: {
    include: ["src/lib/analytics-authenticity.db.test.ts"],
    fileParallelism: false,
    env: {
      DATABASE_URL: databaseUrl,
      DIRECT_URL: directUrl,
      G7_ANALYTICS_AUTHENTICITY_SCHEMA: schema,
    },
  },
});
