# QUAL-2026-08-07-08：WP151 source attribution
...
在不執行 WP151 live runner、不建立 disposable schema、不啟動 Browser 或 provider 的前提下，補上：
...
- `node --test --experimental-test-coverage scripts/wp151-public-unavailable-browser-runner.test.mjs`：`16/16 PASS`、0 failed、0 skipped。
- WP151 source-entry process coverage：lines `61.61%`、branches `77.66%`、functions `80.77%`。
- scoped ESLint：PASS。
- `git diff --check`：PASS。
