...
| 既有演練 | 隔離 PostgreSQL 17 container 邏輯還原通過，RTO 8.51 秒 | 驗證了 SQL 還原步驟，不能當作 Supabase project restore 證據。 |
| WSL direct connection | 不可達 | `db.<project-ref>.supabase.co:5432` 只解析 IPv6，而目前 WSL 沒有 IPv6 egress。 |
| WSL 備份工具 | 可用 | 以 Linux 原生 `npx supabase` 搭配 Docker 內的 PostgreSQL client 執行；host 沒有安裝 `pg_dump` / `psql`。 |
...
在未升級方案前，Staging 的有效策略是「加密邏輯匯出 + 隔離目標還原演練」。每次至少產生三個主備份檔：

1. `roles.sql`：custom roles。
2. `schema.sql`：schema、constraints、indexes、functions 與 policies。
3. `data.sql`：`public` schema 的資料。

CelebrateDeal 使用 Prisma；migration history 位於 `public._prisma_migrations`，因此已包含在 `data.sql`，不應另外假設存在 `supabase_migrations` schema。
...
建立目錄與權限後，使用 Supabase CLI 依序匯出。CLI 的預設 dump 不含資料與 custom roles，因此三個主備份命令都不可省略。
...
4. 若 target 是一般 PostgreSQL container，先建立最小的 Supabase 相容 no-login roles。Supabase target 本身已管理這些 roles，不必重建。不得授予 LOGIN、SUPERUSER 或額外權限：
...
5. 以單一 transaction 還原 roles、schema、data。任何一個 SQL error 都要停止，不能略過。`data.sql` 已含 Prisma 的 `_prisma_migrations`。
...
8. 以 aggregate query 抽查 `Vendor`、`User`、`VendorMember`、`PaymentTransaction`、`RefundRecord`、`Settlement`、`PayoutBatch`、`PayoutItem`、`WebhookEvent`、`AuditLog` 的筆數與關聯完整性。比較結果須依資料表名稱或其他穩定 key 正規化，不能直接比較不同 collation target 的原始排序文字。只存 pass / fail 與筆數差異，不匯出個資或金流內容。
9. 量測從開始還原到上述驗證完成的時間，寫入演練紀錄。RTO 必須以該次 target 演練為準；本機 container 的測量值不能直接當作 Supabase project RTO。
...
- [ ] `roles.sql`、`schema.sql`、`data.sql` 都建立成功；Prisma `_prisma_migrations` 已包含在 `data.sql`。
...
本輪已完成 Staging project 健康、方案、PITR、可列出備份能力、WSL IPv6 及工具可用性確認，並完成一次對隔離 PostgreSQL 17 container 的加密邏輯備份與還原演練。細節見 [2026-07-22 Staging 演練紀錄](./staging-backup-restore-drill-2026-07-22.md)。

本輪已在不升級方案的前提下完成本機隔離 target 演練。若未來要取得 Supabase 平台級 restore 證據，才需要人工決定其中一種 target：
...
目前 Staging 備份與還原成熟度為「加密邏輯備份與本機隔離還原已驗證，平台級 restore drill 尚未驗證」。
