...
|---|---|---|---|---|---|
| Contract drift、coverage merge 與 build／CI blocker | `PASS_LOCAL_ONLY` | `docs/ai-team/evidence/release-evidence-bundle-current-status-20260821.json`、`docs/ai-team/evidence/rel-20260821-external-smoke-owner-authorization.md`、`docs/ai-team/evidence/rel-20260821-external-provider-source-binding.md`、`docs/ai-team/evidence/rel-20260821-staging-release-evidence-contract.md`、`docs/ai-team/evidence/rel-20260821-local-browser-performance-stabilization.md`；current release handoff contract `1/1`，且已納入 `.github/workflows/ci.yml` 的 `Current release handoff evidence consistency` step；Node TAP `841/841`；combined coverage `404 files passed／1 skipped`、`3090 passed／1 skipped`，exit `0`，statements／branches／functions／lines=`64.75／64.49／71.04／69.65`，高於 `63／57／60／65`；focused Browser `11/11`；`npm run e2e:performance` `5/5`；owner authorization contract `8/8`；PayUni runner suite `36/36`；PayUni deployment-boundary synthetic test `33/33`；CI workflow 已加入 PayUni binding、release readiness、readiness truth、staging migration evidence、staging migration receipt validation、staging release evidence aggregation、staging release evidence receipt validation、human owner acceptance evidence、release evidence bundle、current release handoff evidence consistency、non-Production owner authorization、external smoke output safety、external provider evidence 與 provider receipt validator contract steps；`test:release-readiness` `5/5`；staging migration evidence contract `7/7`；staging migration receipt validator `9/9`；staging release evidence aggregate `9/9`；staging release receipt validator `9/9`；human owner acceptance validator `10/10`；release evidence bundle validator `12/12`；external smoke safety `14/14`；external provider evidence `13/13`；provider receipt validator `8/8`；secret scan、controlled production build、local release verifier、`npm audit --omit=dev --audit-level=high` 與 diff check 均通過；AI Team server `7/7`、resilience 與 backup tooling static checks 通過；current RC 另通過 MFA helper／route targeted tests、修改檔案 ESLint／typecheck，並將原 MFA Browser blocker 修正 | 2026-08-21 唯讀檢查顯示遠端 branch head 仍是舊提交 `c2aa2201`；最新列出的 GitHub Actions run `32209974601` 的 `Production dependency audit` step 為 `failure`，沒有 current RC `96407f8` 的 run | `TRACKED_PROJECT_REQUIREMENT` | 由 owner 依核准流程觸發 current RC 的 remote CI；未取得 run 前維持 `PASS_LOCAL_ONLY` |
| 乾淨 release candidate | `PASS_LOCAL_ONLY` | release candidate `96407f8`；current completion audit checkpoint；Git status、staged index、diff check clean | 未 push、未 merge、未部署；staging 尚未證明使用此 source lineage | `TRACKED_PROJECT_REQUIREMENT` | 以受控 non-Production deployment 流程重新建立 exact lineage，禁止貼出 credential |
| Release evidence bundle aggregation | `PASS_LOCAL_ONLY` | `scripts/validate-release-evidence-bundle.mjs` contract `12/12`；current-RC status bundle `docs/ai-team/evidence/release-evidence-bundle-current-status-20260821.json` 以 source `96407f8` 通過 CLI，結果為 `INCOMPLETE`；13 gate exact-set、source-lineage、opaque reference、non-Production boundary 與 `GO` fail-closed aggregation | 這是如實記錄目前缺口的 baseline bundle，不是所有 gate 已完成的 release candidate；各 external／staging／PayUni／human receipt 仍未完成 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | owner 收集真實 sanitized receipts 後更新 current-RC bundle；所有 gate PASS 前維持 `NO_GO` |
...
current source RC `96407f8` 的完整 Browser release gate 尚未通過。前一個 RC `6e3eddb` 的 blocker 是 owner MFA flow 的 Next 16 Server Action redirect transport；本 RC 已以共用 helper、native POST route 與 browser-Origin-preserving 303 redirect 修正，且 targeted helper／route tests、修改檔案 ESLint／typecheck、static owner case `1/1` 與 standalone accessibility `8/8` 通過。完整 E2E 的最新實際結果為 `126 passed／1 failed／11 did not run`；失敗改為 G7-04 出貨流程的 `?updated=shipping` navigation assertion，單獨重跑該案例為 `1/1`，另一次 commerce-orders suite 則在 fixture 建立階段遇到 PostgreSQL `40P01 deadlock detected`。完整 evidence 見 [`rel-20260821-local-browser-release-gate.md`](../ai-team/evidence/rel-20260821-local-browser-release-gate.md)。
因此目前只能判定 MFA blocker 已關閉，但 current Browser gate 仍為 `BLOCKED`；不能以 focused／standalone PASS 或單次完整 suite 結果取代穩定的 full gate。
...
2026-08-21 以 GitHub CLI 進行唯讀查詢：遠端 `codex/one-stop-webinar-flow` branch head 為舊提交 `c2aa2201`；最新列出的 `ci.yml` run `32209974601` 在該提交上的 `Production dependency audit` step 以 `failure` 結束。查詢結果沒有 current RC `6e3eddb` 的 remote run，因此 current release candidate 的 GitHub Actions 狀態仍為 `NOT_PROVEN`。本次沒有 push、workflow dispatch、deployment 或任何外部 side effect。
