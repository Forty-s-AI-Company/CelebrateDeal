...
|---|---|---|---|---|---|
| Contract drift、coverage merge 與 build／CI blocker | `PASS_LOCAL_ONLY` | current source `a28bf33` 的 unit suite 為 `410 files passed／1 skipped`、`3106 passed／1 skipped`；Node TAP `843/843`；combined coverage threshold PASS，statements／branches／functions／lines=`64.84／64.51／71.02／69.75`；strict-index typecheck、lint、controlled production preflight `0`、Next build `0` 與 controlled build `PASS` 均通過，build receipt 見 [`rel-20260821-controlled-production-build.md`](../ai-team/evidence/rel-20260821-controlled-production-build.md)。完整 Browser `138 passed／0 failed／0 skipped` 的 application/runtime evidence 仍來自 parent source `9cd7473`；controlled build 執行來源為 parent checkpoint `c308363`，`a28bf33` 只新增 evidence contract 與 policy lineage assertion | 2026-08-21 唯讀檢查顯示遠端 branch head 仍是舊提交 `c2aa2201`；沒有 current RC `a28bf33` 的 remote run；controlled synthetic preflight 不等於實際 Production binding，正式 `CRON_SECRET`、獨立 `CSRF_SECRET`、`LIVE_CHAT_INGRESS_SECRET`、durable rate limit 與其他正式設定仍未完成可驗證設定 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | 先由 owner 依核准流程觸發 current RC 的 remote CI，並補齊 non-Production／Production preflight evidence；未取得 run 與正式 binding receipt 前維持 `PASS_LOCAL_ONLY` |
| 乾淨 release candidate | `PASS_LOCAL_ONLY` | release candidate `a28bf33`；source tests、CAT10 evidence lineage contract、controlled production preflight/build 與既有 application/runtime Browser evidence 已核對；完成 docs sync 後以 `git status`、staged index、diff check 確認 clean | 未 push、未 merge、未部署；staging 尚未證明使用此 source lineage | `TRACKED_PROJECT_REQUIREMENT` | 以受控 non-Production deployment 流程重新建立 exact lineage，禁止貼出 credential |
| Release evidence bundle aggregation | `PASS_LOCAL_ONLY` | `scripts/validate-release-evidence-bundle.mjs` contract `12/12`；current-RC status bundle `docs/ai-team/evidence/release-evidence-bundle-current-status-20260821.json` 以 source `a28bf33` 驗證，結果仍為 `INCOMPLETE`；13 gate exact-set、source-lineage、opaque reference、non-Production boundary 與 `GO` fail-closed aggregation | 這是如實記錄目前缺口的 baseline bundle，不是所有 gate 已完成的 release candidate；各 external／staging／PayUni／human receipt 仍未完成 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | owner 收集真實 sanitized receipts 後更新 current-RC bundle；所有 gate PASS 前維持 `NO_GO` |
