...
|---|---|---|---|---|---|
| Contract drift、coverage merge 與 build／CI blocker | `PASS_LOCAL_ONLY` | current source `9cd7473` 的 unit suite 為 `410 files passed／1 skipped`、`3106 passed／1 skipped`；Node TAP `841/841`；combined coverage threshold PASS，statements／branches／functions／lines=`64.80／64.48／71.01／69.71`；strict-index typecheck、lint 與 full Browser `138 passed／0 failed／0 skipped` 均通過 | 2026-08-21 唯讀檢查顯示遠端 branch head 仍是舊提交 `c2aa2201`；最新列出的 GitHub Actions run `32209974601` 的 `Production dependency audit` step 為 `failure`，沒有 current RC `9cd7473` 的 run；production preflight 所需的 `CRON_SECRET`、獨立 `CSRF_SECRET` 與 `LIVE_CHAT_INGRESS_SECRET` 尚未完成可驗證設定 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | 先由 owner 依核准流程觸發 current RC 的 remote CI，並補齊 non-Production／Production preflight evidence；未取得 run 與 preflight receipt 前維持 `PASS_LOCAL_ONLY` |
