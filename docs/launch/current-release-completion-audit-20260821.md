...
|---|---|---|---|---|---|
| Contract drift、coverage merge 與 build／CI blocker | `PASS_LOCAL_ONLY` | current source `9cd7473` 的 unit suite 為 `410 files passed／1 skipped`、`3106 passed／1 skipped`；Node TAP `841/841`；combined coverage threshold PASS，statements／branches／functions／lines=`64.80／64.48／71.01／69.71`；strict-index typecheck、lint、full Browser `138 passed／0 failed／0 skipped` 與 controlled production build `PASS` 均通過，build receipt 見 [`rel-20260821-controlled-production-build.md`](../ai-team/evidence/rel-20260821-controlled-production-build.md) | 2026-08-21 唯讀檢查顯示遠端 branch head 仍是舊提交 `c2aa2201`；最新列出的 GitHub Actions run `32209974601` 的 `Production dependency audit` step 為 `failure`，沒有 current RC `9cd7473` 的 run；production preflight 所需的 `CRON_SECRET`、獨立 `CSRF_SECRET` 與 `LIVE_CHAT_INGRESS_SECRET` 尚未完成可驗證設定 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | 先由 owner 依核准流程觸發 current RC 的 remote CI，並補齊 non-Production／Production preflight evidence；未取得 run 與 preflight receipt 前維持 `PASS_LOCAL_ONLY` |
| 乾淨 release candidate | `PASS_LOCAL_ONLY` | release candidate `9cd7473`；source tests 與 local Browser gate 已重跑；完成 docs sync 後以 `git status`、staged index、diff check 確認 clean | 未 push、未 merge、未部署；staging 尚未證明使用此 source lineage | `TRACKED_PROJECT_REQUIREMENT` | 以受控 non-Production deployment 流程重新建立 exact lineage，禁止貼出 credential |
| Release evidence bundle aggregation | `PASS_LOCAL_ONLY` | `scripts/validate-release-evidence-bundle.mjs` contract `12/12`；current-RC status bundle `docs/ai-team/evidence/release-evidence-bundle-current-status-20260821.json` 以 source `9cd7473` 驗證，結果仍為 `INCOMPLETE`；13 gate exact-set、source-lineage、opaque reference、non-Production boundary 與 `GO` fail-closed aggregation | 這是如實記錄目前缺口的 baseline bundle，不是所有 gate 已完成的 release candidate；各 external／staging／PayUni／human receipt 仍未完成 | `TRACKED_PROJECT_REQUIREMENT` / `DIRECT_PRODUCTION_RISK` | owner 收集真實 sanitized receipts 後更新 current-RC bundle；所有 gate PASS 前維持 `NO_GO` |
...
2026-08-21 以 GitHub CLI 進行唯讀查詢：遠端 `codex/one-stop-webinar-flow` branch head 為舊提交 `c2aa2201`；最新列出的 `ci.yml` run `32209974601` 在該提交上的 `Production dependency audit` step 以 `failure` 結束。查詢結果沒有 current RC `9cd7473` 的 remote run，因此 current release candidate 的 GitHub Actions 狀態仍為 `NOT_PROVEN`。本次沒有 push、workflow dispatch、deployment 或任何外部 side effect。
