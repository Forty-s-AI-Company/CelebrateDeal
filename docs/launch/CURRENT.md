...
- 最新已合併 master 為 `7becddfd141ac96f3e0d94555c268d31e315247b`（受保護 PR #274–#283）。#283 的兩條完整 `quality` [run 36056518227](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36056518227)／[run 36056523535](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36056523535) 與兩個 Vercel Preview 檢查通過；合併前的證據只適用於該 PR commit。[受保護 Preview 身分檢查](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36035415243) 成功。
...
| 固定 staging 登入後頁面煙測 | **BLOCKED** | [受保護 run 36059209829](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36059209829)：合成 session 已建立；桌機／手機共 8 個路由皆 200 且最後停在預期路徑，沒有 page error／同站 5xx，但預期標題／資料未出現。runner 擋下 47 個非 GET 請求（API 8、其他 39）；不能把 200 視為頁面可用，也尚未單獨證明原因是 schema drift。此檢查不涵蓋正常登入、Funnel 保存或付款 |
...
| staging Prisma migration | **DRIFT：58／79** | [受保護唯讀診斷](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36059197795) 證實精確缺最後 21 個，完整前綴且 checksum 相符；沒有套用 migration。SQL 初步靜態掃描未見頂層 DROP／DELETE／TRUNCATE／UPDATE，但約束與既有資料相容性仍需檢查 |
