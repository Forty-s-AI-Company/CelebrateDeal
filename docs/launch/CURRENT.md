...
| Dashboard 導頁與關鍵 JS／CSS | **BLOCKED：目前來源** | [run 36270832094](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36270832094) 對 `1909c71a` 的首個 desktop Dashboard 逾時：document 回 2xx，外框與 KPI 已顯示，明細未就緒，`DOMContentLoaded` 未發生；0 關鍵資源失敗、頁面錯誤或同站 5xx，合成 session 撤銷。前一來源 [run 36264888394](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36264888394) 曾 desktop/mobile 十頁 PASS，但不能替目前來源驗收；這是明細串流／讀取阻塞的新定位，不應盲目重跑 |
...
| R2 staging 圖片上傳與公開讀取 | **PASS：目前來源** | [run 36270832220](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36270832220) 對 `1909c71a` 完成一筆合成圖片的預簽、PUT、完成回報及公開 r2.dev GET；bucket 與公開讀取均驗證，0 瀏覽器錯誤或不安全寫入，唯一外部讀取被攔截，session 撤銷。`celebrate-deal-staging` bucket 公開網址只供合成資料，憑證限此 bucket，CORS 限固定 staging origin 的 PUT |
