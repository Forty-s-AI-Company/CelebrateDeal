...
| staging Prisma migration | **DRIFT：58／79** | [最新受保護唯讀診斷](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36082927966) 再次確認缺最後 21 個、`Vendor.enabledFeatureModules` 不存在；完整前綴且 checksum 相符，`databaseWrites=0`。[隔離重播](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36086949395) 已證明本次快照的 21 個 SQL 可執行，但固定 DB 尚未變更 |
| staging 隔離 migration 相容性 | **PASS，僅一次性還原** | [受保護 run 36086949395](https://github.com/Forty-s-AI-Company/CelebrateDeal/actions/runs/36086949395)：metadata、各表筆數、extensions、21 個 SQL、目標 schema 與原 history 均通過；`databaseWrites=0`、`replayedMigrationCount=21`，未執行 Prisma migrate deploy |
...
下一步先確認可保留的非 Production 回復備份或明確的 disposable staging 資料邊界，再用受保護流程修復 staging schema。已通過的隔離重播不會留下 dump，不能單獨充當回復方案。舊本機 `codex/staging-migration-apply-gate` 草稿已被複審否決；新 [PR #299](https://github.com/Forty-s-AI-Company/CelebrateDeal/pull/299)（已納入 [整合 PR #304](https://github.com/Forty-s-AI-Company/CelebrateDeal/pull/304)） 改用現行 replay receipt、固定 staging project、master／固定來源 migration 和 schema tree 比對、套用前後 history 與 schema catalog 檢查，並改由單次手動 dispatch 輸入資料處置確認，不再依賴可能從 repository／organization 繼承的變數。本機測試 9/9、YAML 解析與 secret scan 通過；整合程式碼本身不會執行 migration；固定 staging DB 仍未套用。固定 staging 的資料處置仍未獲明確確認，因此不可 dispatch migration apply。PR #304 同時整合唯讀 provider 探測、登入後手機／桌機與 React 互動煙測，以及 PayUni Sandbox 訂單／重複 callback 證據工具；目前都尚無固定 staging 的外部 PASS 收據。schema 修復後重跑同一來源的登入後核心瀏覽器旅程，並區分 fixture 503 是部署 source SHA 設定還是寫入例外。只有 fixture 根因解決後才考慮下一次 Sandbox 交易，且需將成功付款、callback、訂單 DB readback／重複 callback 冪等性串在同一 lineage。未證實的項目保持 `NOT_PROVEN`。較細的未製作功能見 [下一輪清單](NEXT-CYCLE.md)；歷史 checkpoint 見 [執行紀錄](goal-progress-20260924.md)。
