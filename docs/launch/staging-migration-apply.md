...
2. 先取得 owner 對**固定 staging 現有資料全部可視為可拋棄的合成測試資料**的明確確認。未確認時不能設定 GitHub Environment `Preview – celebrate-deal-staging` 的 `STAGING_DATA_DISPOSITION=ALL_DISPOSABLE_SYNTHETIC`；目前此變數不存在，無保留備份的流程會擋下寫入。確認且設定完成後，以該 run ID 手動觸發 `Staging migration apply`。workflow 會向 GitHub API 核對 producer 的 `workflow_dispatch`、成功狀態、`master`、精確 workflow 路徑與相同 commit；它只接受 artifact `celebratedeal-staging-isolated-migration-replay` 內的 `celebratedeal-staging-migration-replay.json`。
