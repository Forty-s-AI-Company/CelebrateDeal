...
- Cloudflare Stream API client、direct upload、Live Input、signed webhook 與 external smoke 已在 repo 內實作，並非僅有資料結構或 UI 流程預留；但 Cloudflare dashboard 的 token scope／account mapping、真實 dashboard callback 與 production 驗收仍需外部環境完成。目前 tracked report 亦記錄 direct upload 曾回 `code=10000 Authentication error`，因此不得視為外部 callback、production 憑證或真實 Cloudflare 資源已全部驗收成功。
- Prisma 已切換為 PostgreSQL datasource，舊 SQLite migrations 已歸檔；正式 SaaS 後續以 Supabase Postgres baseline migration 為主。
- 互動腳本編輯器已支援新增、刪除留言列與 HTML drag/drop 排序；UI 以 `src/components/interaction-script-form.tsx` 的 drag handlers 處理拖拉。排序邏輯集中於 `src/lib/interaction-timeline.ts` 的 `reorderInteractionEvents`，而 `src/lib/interaction-timeline.test.ts` 已覆蓋有效重排及無效 index 時維持原順序。現有證據僅涵蓋元件接線與單元測試，未將瀏覽器層 drag/drop E2E 列為已驗證項目。
