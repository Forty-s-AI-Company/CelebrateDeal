...
class LiteRouterTest(unittest.TestCase):
    def test_router_json_is_lite(self) -> None:
        ...
        self.assertEqual(config["agents"]["worker"]["reasoning_effort"], "max")
        self.assertEqual(config["agents"]["worker"]["reasoning_lock"], "max")
    ...
    def test_route_task_uses_fixed_routes_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(worker["reasoning_lock"], "max")
    ...
    def test_native_reasoning_is_adaptive_and_bounded(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual((trivial["difficulty"], trivial["reasoning_effort"]), ("trivial", "max"))
            self.assertEqual((routine["difficulty"], routine["reasoning_effort"]), ("routine", "max"))
