...
$snapshot = Join-Path $temp "CelebrateDeal-WP59-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP59-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-59-accountant-videos-new-direct-url-$id"
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31059'
    E2E_BASE_URL = 'http://127.0.0.1:31059'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp59-browser.json')
...
function Get-DirtyManifest([string[]]$status) {
  $owned = @(
    '.ai-team/scripts/Invoke-Wp59AccountantVideosNewDirectUrlQa.ps1',
    'tests/e2e/accountant-videos-new-direct-url.spec.ts'
  )
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/videos/new/page.tsx' -Raw
  if ($page.IndexOf('await requireVendorManager();') -lt 0 -or
      $page.IndexOf('await requireVendorManager();') -gt $page.IndexOf('<VideoForm />')) {
    throw 'Videos-new page guard order drift.'
  }
  $form = Get-Content -LiteralPath 'src/components/video-form.tsx' -Raw
  foreach ($required in @('action={upsertVideoAction}', '<CsrfField />', 'name="videoUrl"', 'name="thumbnailUrl"')) {
    if (-not $form.Contains($required)) { throw "Video form contract drift: $required" }
  }
  ...
  if ($slice.IndexOf('await assertServerActionSecurity(formData);') -gt $slice.IndexOf('const vendor = await requireVendorManager();')) {
    throw 'Video action guard order drift.'
  }
  foreach ($required in @('vendorId: vendor.id', 'sourceType: "url"', 'status: "ready"')) {
    if (-not $slice.Contains($required)) { throw "Video create contract drift: $required" }
  }
  foreach ($providerField in @('cloudflareStreamUid', 'cloudflareLiveInputUid', 'cloudflarePlaybackId', 'cloudflareReadyToStream', 'liveInputStatus')) {
    if ($slice.Contains($providerField)) { throw "Provider-owned field became writable: $providerField" }
  }
}

function Preflight {
  if ($schema -notmatch '^wp59_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) { throw 'WP-59 owned paths missing.' }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-59 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp59-bootstrap.sql'
    ...
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp59-bootstrap.sql') $run) -ne 0) { throw 'Bootstrap failed.' }
    $schemaCreated = $true
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-videos-new-direct-url.spec.ts') $run) -ne 0) { throw 'Lint failed.' }
    if ((Invoke-Child 'auth-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/auth.test.ts') $run) -ne 0) { throw 'Auth tests failed.' }
    if ((Invoke-Child 'video-form-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/components/video-form.test.tsx') $run) -ne 0) { throw 'Video form tests failed.' }
    if ((Invoke-Child 'video-action-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/actions.test.ts', '-t', 'upsertVideoAction') $run) -ne 0) { throw 'Video action tests failed.' }
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-videos-new-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) { throw 'Browser gate failed.' }
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares full vendor/video rows and counts; it also records no post-login POST or non-loopback request.'
    result = 'UNCHANGED'
  })
} catch {
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp59-cleanup.sql'
      ...
      Push-Location $snapshot
      try {
        $cleanupCode = Invoke-Child 'database-cleanup' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp59-cleanup.sql') $run
      } finally {
        Pop-Location
      }
...
Write-Output "WP-59 report: $report"
