...
# WP-36 never reads or copies .env files. Every child receives this bounded,
...
$snapshotRoot = [IO.Path]::GetFullPath((Join-Path $tempRoot "CelebrateDeal-WP36-$runId"))
$runtimeRoot = [IO.Path]::GetFullPath((Join-Path $tempRoot "CelebrateDeal-WP36-runtime-$runId"))
$snapshotMarker = ".wp36-snapshot-$runId"
$runtimeMarker = ".wp36-runtime-$runId"
$schema = "wp36_$runId"
$schemaMarker = "wp36:$runId"
$reportDirectory = Join-Path ([IO.Path]::GetFullPath((Join-Path $workspaceRoot $ReportRoot))) "wp-36-admin-cross-tenant-affiliate-edit-$runId"
$logDirectory = Join-Path $workspaceRoot ".ai-team\logs\wp-36\$runId"
...
if (-not $snapshotRoot.StartsWith($tempRoot, [StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw "WP-36 temporary paths must be distinct children of the system temporary directory."
}
if ($schema -notmatch '^wp36_[a-z0-9_]+$') { throw "WP-36 schema name is invalid." }
...
function Sanitize-Text([string]$Value) {
    ...
    $safe = $safe -replace '(?i)wp36-(?:own|foreign)-[a-f0-9]+', '[REDACTED_SYNTHETIC_CANARY]'
    return $safe
}
...
function Assert-GitBaseline([string]$Branch, [string]$Head, [string]$Phase) {
    if ($Branch -ne $approvedBranch) { throw "WP-36 branch mismatch during $Phase." }
    if ($Head -ne $approvedHead) { throw "WP-36 HEAD mismatch during $Phase." }
}
...
function Assert-OwnershipInventory([string[]]$Status, [string]$Phase) {
    ...
    if (($actual -join "`n") -ne ($expected -join "`n")) {
        ...
        throw "WP-36 ownership mismatch during $Phase. unexpected=$($unexpected -join ';'); missing=$($missing -join ';')"
    }
...
function Assert-ManifestStable($Before, $After, [string]$Name) {
    if (($Before | ConvertTo-Json -Compress) -ne ($After | ConvertTo-Json -Compress)) { throw "WP-36 $Name changed during the execution window." }
}

function Assert-Rejected([scriptblock]$Action, [string]$Name) {
    $rejected = $false
    try { & $Action } catch { $rejected = $true }
    if (-not $rejected) { throw "WP-36 preflight self-test accepted: $Name" }
}
...
function Invoke-PreflightSelfTest {
    ...
    Assert-Rejected { [void](Assert-OwnershipInventory @($expected + "?? __wp36_unknown_path__") "self-test-unknown") } "unknown path"
    ...
    Write-Output "WP-36 preflight self-test PASS: fixed branch/HEAD plus 38 PRESERVE_ONLY and 2 WP_OWNED guards."
}
...
function Remove-MarkedTemporaryDirectory([string]$Path, [string]$Marker, [string]$Kind) {
    ...
    if (-not $Path.StartsWith($normalizedTempRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        throw "WP-36 $Kind cleanup marker or path check failed."
    }
    ...
    throw "WP-36 $Kind cleanup left an unexpected path."
}

function Assert-Preflight([System.Collections.IDictionary]$GitState, $ProtectedManifest) {
    if (@($GitState.staged).Count -ne 0) { throw "WP-36 requires an empty staged index." }
...
function Get-SyntheticEnvironment {
    ...
    return [ordered]@{
        ...
        E2E_PORT = "31036"
        E2E_BASE_URL = "http://127.0.0.1:31036"
        E2E_SMOKE_TEST_EMAIL = "wp36-synthetic@celebratedeal.test"
        RATE_LIMIT_PROVIDER = "memory"
        PAYMENT_PROVIDER = "demo"
        JOB_SECRET = "wp36-job-secret-at-least-16-chars"
        CSRF_SECRET = "wp36-csrf-secret-at-least-16-chars"
...
function Assert-DatabaseSafety([System.Collections.IDictionary]$Environment) {
    ...
    if ($uri.Host -ne "127.0.0.1" -or $uri.Port -ne 54329 -or $uri.AbsolutePath.Trim('/') -ne "celebratedeal_ci" -or $query["schema"] -ne $schema -or $schema -notmatch '^wp36_[a-z0-9_]+$') {
        throw "WP-36 database boundary rejected."
    }
}
...
try {
    ...
    Write-Utf8File (Join-Path $snapshotRoot $snapshotMarker) "WP-36 snapshot marker: $runId"
    Write-Utf8File (Join-Path $runtimeRoot $runtimeMarker) "WP-36 runtime marker: $runId"
    ...
    Write-Json "environment-safety.json" ([ordered]@{ work_package = "WP-36"; source_env_contents_read = $false; source_env_files_copied = $false; synthetic_environment_only = $true; database = [ordered]@{ host = "127.0.0.1"; port = 54329; database = "celebratedeal_ci"; schema = $schema; schema_pattern = "^wp36_[a-z0-9_]+$" } })
    ...
    $bootstrapSql = Join-Path $snapshotRoot "wp36-bootstrap.sql"
    Write-Utf8File $bootstrapSql "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$schemaMarker';`n"
    if ((Invoke-IsolatedCommand "database-bootstrap" $node @($prismaCli, "db", "execute", "--schema", "prisma/schema.prisma", "--file", ".\wp36-bootstrap.sql") $syntheticEnvironment) -ne 0) { throw "Schema bootstrap failed." }
    $schemaCreated = $true
    ...
    if ((Invoke-IsolatedCommand "spec-eslint" $node @("node_modules/eslint/bin/eslint.js", "tests/e2e/admin-cross-tenant-affiliate-edit.spec.ts") $syntheticEnvironment) -ne 0) { throw "WP-36 spec ESLint failed." }
    if ((Invoke-IsolatedCommand "auth-unit" $node @("node_modules/vitest/vitest.mjs", "run", "src/lib/auth.test.ts") $syntheticEnvironment) -ne 0) { throw "WP-36 auth unit gate failed." }
    if ((Invoke-IsolatedCommand "browser-e2e" $node @("node_modules/@playwright/test/cli.js", "test", "tests/e2e/admin-cross-tenant-affiliate-edit.spec.ts", "--project=chromium", "--retries=0") $syntheticEnvironment) -ne 0) { throw "Release-mode Browser gate failed." }
    ...
    Write-Json "db-invariant-summary.sanitized.json" ([ordered]@{ source = "Browser spec DB assertions"; own_affiliate_snapshot = "unchanged"; foreign_affiliate_snapshot = "unchanged"; own_affiliate_click_snapshot = "unchanged"; foreign_affiliate_click_snapshot = "unchanged"; owner_vendor_affiliate_count = "unchanged"; foreign_vendor_affiliate_count = "unchanged"; own_affiliate_click_count = "unchanged"; foreign_affiliate_click_count = "unchanged"; fixture_cleanup = "asserted by spec finally" })
} catch {
    ...
    if ($schemaCreated) {
        try {
            $syntheticEnvironment = Get-SyntheticEnvironment
            Assert-DatabaseSafety $syntheticEnvironment
            $cleanupSql = Join-Path $snapshotRoot "wp36-cleanup.sql"
            $sql = @"
DO `$cleanup`$
BEGIN
  ...
  IF current_database() <> 'celebratedeal_ci' THEN
    RAISE EXCEPTION 'WP-36 database identity mismatch';
  END IF;
  IF (SELECT obj_description(oid, 'pg_namespace') FROM pg_namespace WHERE nspname = '$schema') <> '$schemaMarker' THEN
    RAISE EXCEPTION 'WP-36 schema marker missing or mismatched';
  END IF;
...
"@
            ...
            $npx = (Get-Command npx.cmd -ErrorAction Stop).Source
            $exitCode = Invoke-IsolatedCommand "database-cleanup" $npx @("prisma", "db", "execute", "--schema", "prisma/schema.prisma", "--file", ".\wp36-cleanup.sql") $syntheticEnvironment
    ...
    $summary = [ordered]@{
        work_package = "WP-36"; run_id = $runId; final_runner_error = $runnerFailure
...
Write-Output "WP-36 report: $reportDirectory"
