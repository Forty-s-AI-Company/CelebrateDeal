...
$snapshot = Join-Path $temp "CelebrateDeal-WP67-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP67-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-67-accountant-products-index-direct-url-$id"
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31067'
    E2E_BASE_URL = 'http://127.0.0.1:31067'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp67-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp67AccountantProductsIndexDirectUrlQa.ps1',
    'tests/e2e/accountant-products-index-direct-url.spec.ts'
  )
}
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/products/page.tsx' -Raw
  $guard = $page.IndexOf('const vendor = await requireVendorManager();')
  $query = $page.IndexOf('getDb().product.findMany')
  $render = $page.IndexOf('<PageHeader title="商品管理"', $query)
  if ($guard -lt 0 -or $query -lt $guard -or $render -lt $query) {
    throw 'Products index guard/query/render order drift.'
  }
  foreach ($required in @(
    ...
    'product.isActive ? "上架" : "停用"'
  )) {
    if (-not $page.Contains($required)) { throw "Products index contract drift: $required" }
  }
  ...
  if (-not $shell.Contains('{ href: "/products", label: "商品", icon: Boxes, managerOnly: true }')) {
    throw 'Products navigation manager-only contract drift.'
  }

  $dashboard = Get-Content -LiteralPath 'src/app/(app)/dashboard/page.tsx' -Raw
  if (-not $dashboard.Contains('db.product.count({ where: { vendorId: vendor.id, isActive: true } })')) {
    throw 'Dashboard product count tenant/active contract drift.'
  }
  if (-not $dashboard.Contains('const checklist = dashboardChecklistForRole(')) {
    throw 'Dashboard checklist integration drift.'
  }
  ...
  if ($productStart -lt 0 -or $productEnd -lt 0) { throw 'Dashboard product checklist boundary missing.' }
  $productChecklist = $checklist.Substring($productStart, $productEnd - $productStart)
  foreach ($required in @('href: "/products/new"', 'managerOnly: true')) {
    if (-not $productChecklist.Contains($required)) { throw "Dashboard product checklist drift: $required" }
  }
  ...
  foreach ($required in @(
    ...
    if (-not $form.Contains($required)) { throw "Product form contract drift: $required" }
  }
  ...
  $mutation = $slice.IndexOf('await getDb().product.')
  $redirect = $slice.IndexOf('redirect("/products");')
  if ($security -lt 0 -or $manager -lt $security -or $id -lt $manager -or $mutation -lt $id -or $redirect -lt $mutation) {
    throw 'Product action security/identity/mutation/redirect order drift.'
  }
  foreach ($required in @(
    'optionalExternalUrl(formData, "imageUrl"',
    'optionalExternalUrl(formData, "checkoutUrl"',
    'where: { id, vendorId: vendor.id }',
    'data: { ...data, vendorId: vendor.id }'
  )) {
    if (-not $slice.Contains($required)) { throw "Product action tenant/input contract drift: $required" }
  }
}

function Preflight {
  if ($schema -notmatch '^wp67_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) { throw 'WP-67 owned paths missing.' }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-67 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp67-bootstrap.sql'
    ...
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp67-bootstrap.sql') $run) -ne 0) { throw 'Bootstrap failed.' }
    $schemaCreated = $true
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-products-index-direct-url.spec.ts') $run) -ne 0) { throw 'Lint failed.' }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-products-index-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) { throw 'Browser gate failed.' }
    ...
    if ((Invoke-Child 'git-diff-check' $git @('-C', $root, 'diff', '--check') $run) -ne 0) { throw 'Git diff check failed.' }
  } finally {
    Pop-Location
  }
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares full Vendor, TrackingSetting, User, VendorMember, and active/inactive Product rows plus global, vendor, status, membership, and relation counts; it records no post-authentication POST or non-loopback request.'
    result = 'UNCHANGED'
  })
} catch {
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp67-cleanup.sql'
      ...
      Push-Location $snapshot
      try {
        $cleanupCode = Invoke-Child 'database-cleanup' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp67-cleanup.sql') $run
      } finally {
        Pop-Location
      }
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-67 owned content changed during run.'
    }
  } catch {
    if ($null -eq $failure) { $failure = Safe $_.Exception.Message }
  }
...
Write-Output "WP-67 report: $report"
