...
$snapshot = Join-Path $temp "CelebrateDeal-WP79-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP79-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-79-member-billing-invoices-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-78-member-billing-plans-direct-url-20260730101412629\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp79[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp79-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp79:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP79-$runId"),
    (Join-Path $temp "CelebrateDeal-WP79-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-79 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31079'
    E2E_BASE_URL = 'http://127.0.0.1:31079'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp79-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp79MemberBillingInvoicesDirectUrlQa.ps1',
    'tests/e2e/member-billing-invoices-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-78 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-78 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  ...
  $detailQuery = $detail.IndexOf('const invoice = await getDb().invoice.findFirst({', $detailParams)
  ...
  if ($detailGuard -lt 0 -or $detailParams -lt $detailGuard -or
      $detailQuery -lt $detailParams -or $detailTenant -lt $detailQuery) {
    throw 'Invoice detail guard/params/tenant-query order drift.'
  }
...
function Preflight {
  if ($schema -notmatch '^wp79_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-79 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-79 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31079 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-79 release port 31079 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-79 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp79-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp79-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-invoices-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-invoices-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'Invoice identity, number, month, line-item amounts, status, dates, detail links, and CSV export entry remain absent.'
    result = 'PASS'
  })
  ...
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    ...
    billingInvoicesIndex = 'Static requireVendorFinance-before-tenant-query/reductions/render contract and release-mode Browser denial.'
    ...
    invoiceDetail = 'Six expanded dynamic regression cases cover tenant detail rendering and identifiers; Browser denial does not open detail.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp79-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-79 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-79 report: $report"
