...
$snapshot = Join-Path $temp "CelebrateDeal-WP70-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP70-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-70-accountant-message-templates-new-direct-url-$id"
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31070'
    E2E_BASE_URL = 'http://127.0.0.1:31070'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp70-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp70AccountantMessageTemplatesNewDirectUrlQa.ps1',
    'tests/e2e/accountant-message-templates-new-direct-url.spec.ts'
  )
}
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/messages/templates/new/page.tsx' -Raw
  $guard = $page.IndexOf('await requireVendorManager();')
  $header = $page.IndexOf('<PageHeader title="新增訊息模板"')
  $formRender = $page.IndexOf('<MessageTemplateForm />')
  if ($guard -lt 0 -or $header -lt $guard -or $formRender -lt $header) {
    throw 'Message-template new-page guard/header/form order drift.'
  }
  ...
  foreach ($required in @(
    ...
    'name="body"',
    'name="isActive"',
    'template ? <input type="hidden" name="id"'
  )) {
...
function Preflight {
  if ($schema -notmatch '^wp70_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-70 owned paths missing.'
  }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-70 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp70-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp70-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-message-templates-new-direct-url.spec.ts') $run) -ne 0) {
      throw 'Lint failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-message-templates-new-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares full Vendor, TrackingSetting, User, VendorMember, and active/inactive MessageTemplate rows plus global, tenant, channel, trigger, activity, composite, membership, and relation counts; it records no post-authentication POST or non-loopback request.'
    result = 'UNCHANGED'
  })
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp70-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-70 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-70 report: $report"
