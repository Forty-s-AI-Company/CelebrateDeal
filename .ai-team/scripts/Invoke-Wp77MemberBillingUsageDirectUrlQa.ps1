...
$snapshot = Join-Path $temp "CelebrateDeal-WP77-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP77-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-77-member-billing-usage-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-76-member-affiliate-commissions-direct-url-20260730091808182\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp77[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp77-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp77:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP77-$runId"),
    (Join-Path $temp "CelebrateDeal-WP77-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-77 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31077'
    E2E_BASE_URL = 'http://127.0.0.1:31077'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp77-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp77MemberBillingUsageDirectUrlQa.ps1',
    'tests/e2e/member-billing-usage-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-76 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-76 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/billing/usage/page.tsx' -Raw
  $guard = $page.IndexOf('const { vendor } = await requireVendorFinance("/billing/usage");')
  $monthKey = $page.IndexOf('const monthKey = new Date().toISOString().slice(0, 7);', $guard)
  $monthRange = $page.IndexOf('const { start, end } = monthRange(monthKey);', $monthKey)
  $queries = $page.IndexOf('const [limit, records, currentMonthRecords, subscription, transactions, refundPlatformFeeTotal] = await Promise.all([', $monthRange)
  $limitQuery = $page.IndexOf('getDb().vendorUsageLimit.findUnique', $queries)
  $historyQuery = $page.IndexOf('getDb().usageRecord.findMany', $limitQuery)
  $currentQuery = $page.IndexOf('getDb().usageRecord.findMany', $historyQuery + 1)
  $subscriptionQuery = $page.IndexOf('getDb().vendorSubscription.findFirst', $currentQuery)
  $transactionQuery = $page.IndexOf('getDb().paymentTransaction.findMany', $subscriptionQuery)
  $refundQuery = $page.IndexOf('getDb().refundRecord.aggregate', $transactionQuery)
  $render = $page.IndexOf('title="用量與扣點"', $refundQuery)
  if ($guard -lt 0 -or $monthKey -lt $guard -or $monthRange -lt $monthKey -or
      $queries -lt $monthRange -or $limitQuery -lt $queries -or
      $historyQuery -lt $limitQuery -or $currentQuery -lt $historyQuery -or
      $subscriptionQuery -lt $currentQuery -or $transactionQuery -lt $subscriptionQuery -or
      $refundQuery -lt $transactionQuery -or $render -lt $refundQuery) {
    throw 'Billing usage finance-guard/month/query/render order drift.'
  }
  foreach ($required in @(
    ...
    if (-not $page.Contains($required)) {
      throw "Billing usage query/render contract drift: $required"
    }
  }

  $billing = Get-Content -LiteralPath 'src/lib/billing.ts' -Raw
  $monthRangeStart = $billing.IndexOf('export function monthRange(monthKey: string)')
  $monthRangeEnd = $billing.IndexOf('function ceilCharge', $monthRangeStart)
  if ($monthRangeStart -lt 0 -or $monthRangeEnd -lt 0) {
    throw 'monthRange source boundary missing.'
  }
  $monthRangeSlice = $billing.Substring($monthRangeStart, $monthRangeEnd - $monthRangeStart)
  foreach ($required in @(
    'new Date(Date.UTC(year, month - 1, 1, 0, 0, 0))',
    'new Date(Date.UTC(year, month, 1, 0, 0, 0))',
    'return { start, end };'
  )) {
    if (-not $monthRangeSlice.Contains($required)) {
      throw "UTC half-open month contract drift: $required"
    }
  }
  ...
  foreach ($required in @(
    ...
    if (-not $shell.Contains($required)) {
      throw "Billing usage finance navigation contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp77_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-77 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-77 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31077 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-77 release port 31077 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-77 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp77-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp77-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-usage-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'billing-usage-page-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/(app)/billing/usage/page.test.tsx') $run) -ne 0) {
      throw 'Billing usage page tests failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-usage-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'authorization-boundary.sanitized.json' ([ordered]@{
    ...
    finalStatus = 200
    result = 'DENIED_BEFORE_FINANCE_QUERY_RENDER'
  })
  ...
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'Subscription, usage history, payment transaction, refund, revenue, and platform-fee details remain absent.'
    result = 'PASS'
  })
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    artifactType = 'db-invariant'
    assertion = 'Spec compares non-secret Vendor, TrackingSetting, User, active member VendorMember, BillingPlan, VendorSubscription, VendorUsageLimit, UsageRecord, PaymentTransaction, and RefundRecord rows plus global, tenant, month, status, composite, and relation counts; UserSession is verified only by non-secret count and relation projection.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    billingUsagePage = 'Static requireVendorFinance-before-month/query/render contract, six page unit tests, and release-mode Browser denial.'
    ...
    dbFixtures = 'Browser fixture and DB equality cover ten non-secret entity classes plus safe UserSession counts and relations.'
    billing = 'Eight dynamic billing helper tests cover UTC month range, invoice identity, and refund-adjusted transaction service fees.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp77-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-77 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-77 report: $report"
