...
$snapshot = Join-Path $temp "CelebrateDeal-WP76-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP76-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-76-member-affiliate-commissions-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-75-accountant-affiliate-detail-direct-url-20260730085220210\final-runner-summary.sanitized.json'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp76-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp76:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP76-$runId"),
    (Join-Path $temp "CelebrateDeal-WP76-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-76 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31076'
    E2E_BASE_URL = 'http://127.0.0.1:31076'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp76-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp76MemberAffiliateCommissionsDirectUrlQa.ps1',
    'tests/e2e/member-affiliate-commissions-direct-url.spec.ts'
  )
}
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/affiliates/commissions/page.tsx' -Raw
  $guard = $page.IndexOf('const { vendor } = await requireVendorFinance("/affiliates/commissions");')
  $queries = $page.IndexOf('const [commissions, payouts] = await Promise.all([', $guard)
  $commissionQuery = $page.IndexOf('getDb().affiliateCommission.findMany({', $queries)
  $payoutQuery = $page.IndexOf('getDb().affiliatePayout.findMany({', $commissionQuery)
  $render = $page.IndexOf('title="聯盟分潤"', $payoutQuery)
  if ($guard -lt 0 -or $queries -lt $guard -or $commissionQuery -lt $queries -or
      $payoutQuery -lt $commissionQuery -or $render -lt $payoutQuery) {
    throw 'Affiliate commissions finance-guard/query/render order drift.'
  }
  if ([regex]::Matches($page, 'where: \{ vendorId: vendor\.id \}').Count -lt 2) {
    throw 'Affiliate commissions tenant-bound query contract drift.'
  }
  if ([regex]::Matches($page, 'include: \{ affiliate: true \}').Count -lt 2) {
    throw 'Affiliate commissions relation query contract drift.'
  }
  if ($page.Contains('ledgerEntries') -or $page.Contains('affiliateCommissionLedgerEntry')) {
    throw 'Affiliate commission ledger unexpectedly entered WP-76 coverage.'
  }
  foreach ($required in @(
    'title="聯盟分潤"',
    '>已核准佣金</p>',
    ...
    'payout.monthKey',
    'formatCurrency(payout.adjustmentAmountCents)',
    'formatCurrency(payout.finalAmountCents)'
  )) {
    if (-not $page.Contains($required)) {
      throw "Affiliate commissions render contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp76_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-76 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-76 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31076 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-76 release port 31076 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-76 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp76-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp76-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-affiliate-commissions-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-affiliate-commissions-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares complete Vendor, TrackingSetting, User, active member VendorMember, Affiliate, converted/unconverted AffiliateClick, pending/approved AffiliateCommission, and AffiliatePayout rows plus global, tenant, status, composite, membership, and relation counts.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    commissionsPage = 'Static requireVendorFinance-before-query/render contract plus release-mode Browser denial.'
    ...
    dbFixtures = 'Browser fixture and DB equality cover Vendor, TrackingSetting, User, member membership, Affiliate, AffiliateClick, AffiliateCommission, and AffiliatePayout.'
    commissionLedger = 'Explicitly excluded because the commissions page does not query ledgerEntries; accounting lifecycle remains outside WP-76.'
    affiliateCommission = 'Fourteen dynamic invariant tests.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp76-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-76 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-76 report: $report"
