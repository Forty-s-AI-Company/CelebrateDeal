...
$snapshot = Join-Path $temp "CelebrateDeal-WP64-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP64-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-64-accountant-forms-index-direct-url-$id"
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31064'
    E2E_BASE_URL = 'http://127.0.0.1:31064'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp64-browser.json')
...
function Get-DirtyManifest([string[]]$status) {
  $owned = @(
    '.ai-team/scripts/Invoke-Wp64AccountantFormsIndexDirectUrlQa.ps1',
    'tests/e2e/accountant-forms-index-direct-url.spec.ts'
  )
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/forms/page.tsx' -Raw
  $guard = $page.IndexOf('const vendor = await requireVendorManager();')
  $query = $page.IndexOf('getDb().registrationForm.findMany')
  $render = $page.IndexOf('<PageHeader title="報名表管理"')
  if ($guard -lt 0 -or $query -lt $guard -or $render -lt $query) {
    throw 'Forms index guard/query/render order drift.'
  }
  foreach ($required in @(
    ...
    if (-not $page.Contains($required)) { throw "Forms index contract drift: $required" }
  }

  $actions = Get-Content -LiteralPath 'src/app/actions.ts' -Raw
  $start = $actions.IndexOf('export async function upsertFormAction')
  $end = $actions.IndexOf('export async function upsertTemplateAction', $start)
  if ($start -lt 0 -or $end -lt 0) { throw 'Form action boundary missing.' }
  ...
  $validation = $slice.IndexOf('parseRegistrationFormFields')
  $mutation = $slice.IndexOf('await getDb().registrationForm.')
  if ($security -lt 0 -or $manager -lt $security -or $validation -lt $manager -or $mutation -lt $validation) {
    throw 'Form action security/validation/mutation order drift.'
  }
  if (-not $slice.Contains('vendorId: vendor.id')) { throw 'Form action vendor binding drift.' }

  $shell = Get-Content -LiteralPath 'src/components/app-shell.tsx' -Raw
  if (-not $shell.Contains('{ href: "/forms", label: "報名表", icon: ClipboardList, managerOnly: true }')) {
    throw 'Forms navigation manager-only contract drift.'
  }
}

function Preflight {
  if ($schema -notmatch '^wp64_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) { throw 'WP-64 owned paths missing.' }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-64 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp64-bootstrap.sql'
    ...
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp64-bootstrap.sql') $run) -ne 0) { throw 'Bootstrap failed.' }
    $schemaCreated = $true
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-forms-index-direct-url.spec.ts') $run) -ne 0) { throw 'Lint failed.' }
    ...
    if ((Invoke-Child 'registration-form-fields-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/registration-form-fields.test.ts') $run) -ne 0) { throw 'Registration form field tests failed.' }
    if ((Invoke-Child 'form-action-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/actions.test.ts', '-t', 'upsertFormAction') $run) -ne 0) { throw 'Form action tests failed.' }
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-forms-index-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) { throw 'Browser gate failed.' }
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares full Vendor, TrackingSetting, User, VendorMember, RegistrationForm, and FormSubmission rows plus global, vendor, form, and relation counts; it records no post-login POST or non-loopback request.'
    result = 'UNCHANGED'
  })
} catch {
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp64-cleanup.sql'
      ...
      Push-Location $snapshot
      try {
        $cleanupCode = Invoke-Child 'database-cleanup' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp64-cleanup.sql') $run
      } finally {
        Pop-Location
      }
...
Write-Output "WP-64 report: $report"
