...
$snapshot = Join-Path $temp "CelebrateDeal-WP82-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP82-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-82-member-billing-payouts-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-81-member-billing-settlements-direct-url-20260730112458526\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp82[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp82-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp82:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP82-$runId"),
    (Join-Path $temp "CelebrateDeal-WP82-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-82 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31082'
    E2E_BASE_URL = 'http://127.0.0.1:31082'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp82-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp82MemberBillingPayoutsDirectUrlQa.ps1',
    'tests/e2e/member-billing-payouts-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-81 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-81 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/billing/payouts/page.tsx' -Raw
  $guard = $page.IndexOf('const { vendor } = await requireVendorFinance("/billing/payouts");')
  $query = $page.IndexOf('const batches = await getDb().payoutBatch.findMany({', $guard)
  $outerTenant = $page.IndexOf('where: { items: { some: { vendorId: vendor.id } } }', $query)
  $batchOrder = $page.IndexOf('orderBy: { batchDate: "desc" }', $outerTenant)
  $include = $page.IndexOf('include: {', $batchOrder)
  $itemTenant = $page.IndexOf('where: { vendorId: vendor.id }', $include)
  $includeVendor = $page.IndexOf('include: { vendor: true }', $itemTenant)
  $itemOrder = $page.IndexOf('orderBy: { createdAt: "desc" }', $includeVendor)
  $pending = $page.IndexOf('const pendingCount = batches.flatMap', $itemOrder)
  $render = $page.IndexOf('title="批次出款"', $pending)
  if ($guard -lt 0 -or $query -lt $guard -or $outerTenant -lt $query -or
      $batchOrder -lt $outerTenant -or $include -lt $batchOrder -or
      $itemTenant -lt $include -or $includeVendor -lt $itemTenant -or
      $itemOrder -lt $includeVendor -or $pending -lt $itemOrder -or
      $render -lt $pending) {
    throw 'Billing payouts finance-guard/tenant-query/pending/render order drift.'
  }
  foreach ($required in @(
    ...
    'batch.batchDate',
    'batch.items.length',
    'batch.items.reduce((total, item) => total + item.payoutAmountCents, 0)',
    'maskBankAccount({',
    ...
    if (-not $page.Contains($required)) {
      throw "Billing payouts query/render contract drift: $required"
    }
  }

  $spec = Get-Content -LiteralPath 'tests/e2e/member-billing-payouts-direct-url.spec.ts' -Raw
  if ([regex]::Matches($spec, 'bankAccountEncrypted').Count -ne 2 -or
      [regex]::Matches($spec, 'bankAccountEncrypted:\s*null').Count -ne 2 -or
      $spec.Contains('bankAccountEncrypted: true')) {
    throw 'Browser fixture must store exactly two null encrypted-bank fields and never select the encrypted payload.'
  }

  $prisma = Get-Content -LiteralPath 'prisma/schema.prisma' -Raw
  $vendorStart = $prisma.IndexOf('model Vendor {')
  $vendorEnd = $prisma.IndexOf('model User {', $vendorStart)
  $batchStart = $prisma.IndexOf('model PayoutBatch {')
  $itemStart = $prisma.IndexOf('model PayoutItem {', $batchStart)
  $itemEnd = $prisma.IndexOf('model PaymentAccount {', $itemStart)
  if ($vendorStart -lt 0 -or $vendorEnd -lt $vendorStart -or
      $batchStart -lt 0 -or $itemStart -lt $batchStart -or $itemEnd -lt $itemStart) {
    throw 'Vendor/PayoutBatch/PayoutItem Prisma models missing or reordered.'
  }
  $vendorSlice = $prisma.Substring($vendorStart, $vendorEnd - $vendorStart)
  $batchSlice = $prisma.Substring($batchStart, $itemStart - $batchStart)
  $itemSlice = $prisma.Substring($itemStart, $itemEnd - $itemStart)
  if (-not $vendorSlice.Contains('payoutItems                PayoutItem[]')) {
    throw 'Vendor-to-PayoutItem Prisma relation contract drift.'
  }
  if (-not $batchSlice.Contains('items       PayoutItem[]')) {
    throw 'PayoutBatch-to-PayoutItem Prisma relation contract drift.'
  }
  foreach ($required in @(
    ...
    if (-not $itemSlice.Contains($required)) {
      throw "PayoutItem Prisma relation contract drift: $required"
    }
  }

  $bank = Get-Content -LiteralPath 'src/lib/bank-account.ts' -Raw
  $maskStart = $bank.IndexOf('export function maskBankAccount(details: BankAccountDetails)')
  $maskEnd = $bank.IndexOf('export function maskedStoredBankAccount', $maskStart)
  if ($maskStart -lt 0 -or $maskEnd -lt $maskStart) {
    throw 'Bank-account masking contract missing.'
  }
  $maskSlice = $bank.Substring($maskStart, $maskEnd - $maskStart)
  foreach ($required in @(
    'accountName: visibleName',
    'bankCode: normalized.bankCode',
    'accountNumber: `****${accountCharacters.slice(-4).join("")}`'
  )) {
    if (-not $maskSlice.Contains($required)) {
      throw "Bank-account masking contract drift: $required"
    }
  }
  ...
  foreach ($required in @(
    ...
    if (-not $shell.Contains($required)) {
      throw "Billing payouts finance navigation contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp82_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-82 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-82 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31082 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-82 release port 31082 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-82 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp82-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp82-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-payouts-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'billing-payouts-page-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/(app)/billing/payouts/page.test.tsx') $run) -ne 0) {
      throw 'Billing payouts page tests failed.'
    }
    if ((Invoke-Child 'payout-state-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/payout-state.test.ts') $run) -ne 0) {
      throw 'Payout-state tests failed.'
    }
    if ((Invoke-Child 'bank-account-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/bank-account.test.ts') $run) -ne 0) {
      throw 'Bank-account tests failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-payouts-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'request-boundary.sanitized.json' ([ordered]@{
    ...
    targetPayoutGetRequests = 1
    targetPayoutNonGetRequests = 0
    otherPayoutAdminBillingOrSettlementRequests = 0
    result = 'PASS'
  })
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'All DB-derived PayoutBatch and PayoutItem identities, statuses, dates, synthetic bank display values, masked bank values, amounts, failure reasons, payout UI, and finance links remain absent from the raw redirect body and final Dashboard.'
    result = 'PASS'
  })
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    artifactType = 'db-invariant'
    assertion = 'Spec compares non-secret Vendor, TrackingSetting, User, active member VendorMember, BillingPlan, VendorUsageLimit, PayoutBatch, and PayoutItem rows plus global, tenant, batch, status, composite, and relation counts; bankAccountEncrypted is never selected or serialized; UserSession is verified only by non-secret count and relation projection.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    billingPayouts = 'Static requireVendorFinance-before-tenant-query/pending/render contract, PayoutBatch/PayoutItem/Vendor Prisma relations, masking contract, and release-mode Browser denial.'
    ...
    dbFixtures = 'Browser fixture and DB equality cover eight non-secret entity classes plus safe UserSession counts and relations.'
    payoutPage = 'Two dynamic billing-payout page regression tests cover tenant batch/item scoping and masked rendering.'
    payoutState = 'Three dynamic payout-state regression tests.'
    bankAccount = 'Five dynamic bank-account regression tests; Browser fixture stores no encrypted bank payload.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp82-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-82 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-82 report: $report"
