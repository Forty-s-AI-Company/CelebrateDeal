...
$snapshot = Join-Path $temp "CelebrateDeal-WP71-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP71-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-71-accountant-interaction-roles-new-direct-url-$id"
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31071'
    E2E_BASE_URL = 'http://127.0.0.1:31071'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp71-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp71AccountantInteractionRolesNewDirectUrlQa.ps1',
    'tests/e2e/accountant-interaction-roles-new-direct-url.spec.ts'
  )
}
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/interaction-roles/new/page.tsx' -Raw
  ...
  if ($guard -lt 0 -or $query -lt $guard -or $csrf -lt $query -or $render -lt $csrf) {
    throw 'Interaction-role new-page guard/query/CSRF/render order drift.'
  }
  foreach ($required in @(
    ...
    if (-not $page.Contains($required)) {
      throw "Interaction-role new-page contract drift: $required"
    }
  }
  ...
  if (-not $shell.Contains('{ href: "/interaction-roles", label: "互動角色", icon: Bot, managerOnly: true }')) {
    throw 'Interaction-role navigation manager-only contract drift.'
  }
  ...
  foreach ($required in @(
    ...
    if (-not $checklist.Contains($required)) {
      throw "Dashboard interaction-role checklist contract drift: $required"
    }
  }
  ...
  foreach ($required in @(
    ...
    if (-not $workbench.Contains($required)) {
      throw "Interaction-role workbench contract drift: $required"
    }
  }

  $actions = Get-Content -LiteralPath 'src/app/actions.ts' -Raw
  $start = $actions.IndexOf('export async function importSystemRolesAction')
  $end = $actions.IndexOf('export async function upsertInteractionScriptAction', $start)
  if ($start -lt 0 -or $end -lt 0) { throw 'System role import action boundary missing.' }
  ...
  $lookup = $slice.IndexOf('await db.interactionRole.findMany')
  $create = $slice.IndexOf('await db.interactionRole.createMany')
  $revalidate = $slice.IndexOf('revalidatePath("/interaction-roles");')
  $redirect = $slice.IndexOf('redirect("/interaction-roles");')
  if ($security -lt 0 -or $manager -lt $security -or $lookup -lt $manager -or
      $create -lt $lookup -or $revalidate -lt $create -or $redirect -lt $revalidate) {
    throw 'System role import security/manager/query/create/revalidate/redirect order drift.'
  }
  foreach ($required in @(
    'vendorId: vendor.id',
    ...
    if (-not $slice.Contains($required)) {
      throw "System role import tenant contract drift: $required"
    }
  }
}

function Preflight {
  if ($schema -notmatch '^wp71_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-71 owned paths missing or ownership is ambiguous.'
  }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-71 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp71-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp71-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-interaction-roles-new-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'interaction-role-label-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/interaction-role-label.test.ts') $run) -ne 0) {
      throw 'Interaction role label tests failed.'
    }
    if ((Invoke-Child 'system-role-import-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/actions.test.ts', '-t', 'importSystemRolesAction') $run) -ne 0) {
      throw 'System role import tests failed.'
    }
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-interaction-roles-new-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares full Vendor, TrackingSetting, User, VendorMember, and active/inactive InteractionRole rows plus global, tenant, activity, roleType, label, composite, membership, and relation counts; it records no post-authentication POST or non-loopback request.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    newPage = 'Static guard/query/CSRF/render source contract plus release-mode Browser denial.'
    interactionRolesWorkbench = 'Static field/action contract only; no dedicated component unit suite was claimed.'
    importSystemRolesAction = 'Two targeted dynamic unit tests plus static security/manager/tenant/order contract.'
  })
} catch {
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp71-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-71 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-71 report: $report"
