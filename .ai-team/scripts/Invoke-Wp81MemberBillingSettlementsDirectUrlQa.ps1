...
$snapshot = Join-Path $temp "CelebrateDeal-WP81-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP81-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-81-member-billing-settlements-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-80-member-billing-invoice-detail-direct-url-20260730110154896\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp81[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp81-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp81:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP81-$runId"),
    (Join-Path $temp "CelebrateDeal-WP81-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-81 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31081'
    E2E_BASE_URL = 'http://127.0.0.1:31081'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp81-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp81MemberBillingSettlementsDirectUrlQa.ps1',
    'tests/e2e/member-billing-settlements-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-80 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-80 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/billing/settlements/page.tsx' -Raw
  $guard = $page.IndexOf('const { vendor } = await requireVendorFinance("/billing/settlements");')
  $query = $page.IndexOf('const settlements = await getDb().settlement.findMany({', $guard)
  $tenant = $page.IndexOf('where: { vendorId: vendor.id }', $query)
  $order = $page.IndexOf('orderBy: [{ monthKey: "desc" }]', $tenant)
  $include = $page.IndexOf('include: { payoutBatch: true }', $order)
  $current = $page.IndexOf('const current = settlements[0];', $include)
  $render = $page.IndexOf('title="月結"', $current)
  if ($guard -lt 0 -or $query -lt $guard -or $tenant -lt $query -or
      $order -lt $tenant -or $include -lt $order -or $current -lt $include -or
      $render -lt $current) {
    throw 'Billing settlements finance-guard/tenant-query/current/render order drift.'
  }
  foreach ($required in @(
    ...
    'current.monthlyFeeCents + current.overflowFeeCents + current.paymentServiceFeeCents + current.transactionServiceFeeCents',
    'current.paymentGatewayFeeCents',
    'current.finalPayoutAmountCents',
    'settlement.monthKey',
    'settlement.status',
    'settlement.batchNumber',
    'settlement.payoutDate',
    'settlement.adjustmentAmountCents',
    'settlement.payoutBatch.batchNumber',
    'settlement.payoutBatch.status',
    '>本期成交額</p>',
    '>平台費用</p>',
    '>金流手續費</p>',
    '>預計撥款</p>',
    ...
    if (-not $page.Contains($required)) {
      throw "Billing settlements query/render contract drift: $required"
    }
  }

  $prisma = Get-Content -LiteralPath 'prisma/schema.prisma' -Raw
  $settlementStart = $prisma.IndexOf('model Settlement {')
  $settlementEnd = $prisma.IndexOf('model PayoutBatch {', $settlementStart)
  $payoutBatchEnd = $prisma.IndexOf('model PayoutItem {', $settlementEnd)
  if ($settlementStart -lt 0 -or $settlementEnd -lt $settlementStart -or
      $payoutBatchEnd -lt $settlementEnd) {
    throw 'Settlement/PayoutBatch Prisma models missing or reordered.'
  }
  $settlementSlice = $prisma.Substring($settlementStart, $settlementEnd - $settlementStart)
  $payoutBatchSlice = $prisma.Substring($settlementEnd, $payoutBatchEnd - $settlementEnd)
  foreach ($required in @(
    ...
    if (-not $settlementSlice.Contains($required)) {
      throw "Settlement Prisma relation contract drift: $required"
    }
  }
  if (-not $payoutBatchSlice.Contains('settlements Settlement[]')) {
    throw 'PayoutBatch-to-Settlement Prisma relation contract drift.'
  }
  ...
  foreach ($required in @(
    ...
    if (-not $shell.Contains($required)) {
      throw "Billing settlements finance navigation contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp81_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-81 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-81 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31081 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-81 release port 31081 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-81 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp81-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp81-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-settlements-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'billing-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/lib/billing.test.ts') $run) -ne 0) {
      throw 'Billing unit tests failed.'
    }
    if ((Invoke-Child 'generate-settlement-action-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/actions.test.ts', '-t', 'generateSettlementAction') $run) -ne 0) {
      throw 'Generate-settlement action tests failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-settlements-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'request-boundary.sanitized.json' ([ordered]@{
    ...
    targetSettlementGetRequests = 1
    targetSettlementNonGetRequests = 0
    otherSettlementOrPayoutRequests = 0
    result = 'PASS'
  })
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'All DB-derived Settlement and PayoutBatch identities, months, statuses, amounts, dates, batch metadata, settlement UI, and finance links remain absent from the raw redirect body and final Dashboard.'
    result = 'PASS'
  })
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    artifactType = 'db-invariant'
    assertion = 'Spec compares non-secret Vendor, TrackingSetting, User, active member VendorMember, BillingPlan, VendorUsageLimit, Settlement, and PayoutBatch rows plus global, tenant, month, status, linked/unlinked, composite, and relation counts; UserSession is verified only by non-secret count and relation projection.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    billingSettlements = 'Static requireVendorFinance-before-tenant-query/current/render contract, Settlement/PayoutBatch Prisma relations, and release-mode Browser denial.'
    ...
    billing = 'Eight dynamic billing regression tests.'
    generateSettlement = 'Two targeted dynamic generateSettlementAction regression tests.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp81-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-81 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-81 report: $report"
