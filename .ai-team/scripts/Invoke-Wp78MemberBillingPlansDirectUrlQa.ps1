...
$snapshot = Join-Path $temp "CelebrateDeal-WP78-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP78-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-78-member-billing-plans-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-77-member-billing-usage-direct-url-20260730095422855\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp78[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp78-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp78:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP78-$runId"),
    (Join-Path $temp "CelebrateDeal-WP78-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-78 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31078'
    E2E_BASE_URL = 'http://127.0.0.1:31078'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp78-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp78MemberBillingPlansDirectUrlQa.ps1',
    'tests/e2e/member-billing-plans-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-77 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-77 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/billing/plans/page.tsx' -Raw
  $queryAlias = $page.IndexOf('const queryPromise = props?.searchParams ?? Promise.resolve<BillingPlansSearchParams>({});')
  $guard = $page.IndexOf('const { vendor, member } = await requireVendorFinance("/billing/plans");')
  $canManage = $page.IndexOf('const canManageBilling = member.role === "owner";', $guard)
  $queries = $page.IndexOf('const [plans, currentSubscription, csrfToken, query] = await Promise.all([', $canManage)
  $planQuery = $page.IndexOf('getDb().billingPlan.findMany({', $queries)
  $subscriptionQuery = $page.IndexOf('getDb().vendorSubscription.findFirst({', $planQuery)
  $csrfQuery = $page.IndexOf('canManageBilling ? getCsrfToken() : Promise.resolve("")', $subscriptionQuery)
  $queryUse = $page.IndexOf('queryPromise,', $csrfQuery)
  $queryValue = $page.IndexOf('const status = queryValue(query.status);', $queryUse)
  $render = $page.IndexOf('title="方案"', $queryValue)
  if ($queryAlias -lt 0 -or $guard -lt $queryAlias -or $canManage -lt $guard -or
      $queries -lt $canManage -or $planQuery -lt $queries -or
      $subscriptionQuery -lt $planQuery -or $csrfQuery -lt $subscriptionQuery -or
      $queryUse -lt $csrfQuery -or $queryValue -lt $queryUse -or $render -lt $queryValue) {
    throw 'Billing plans finance-guard/query/CSRF/render order drift.'
  }
  foreach ($required in @(
    'where: { isActive: true }',
    ...
    if (-not $page.Contains($required)) {
      throw "Billing plans query/render contract drift: $required"
    }
  }
  ...
  foreach ($required in @(
    ...
    if (-not $shell.Contains($required)) {
      throw "Billing plans finance navigation contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp78_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-78 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-78 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31078 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-78 release port 31078 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-78 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp78-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp78-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-plans-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'billing-plans-page-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/(app)/billing/plans/page.test.tsx') $run) -ne 0) {
      throw 'Billing plans page tests failed.'
    }
    if ((Invoke-Child 'billing-plans-actions-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/(app)/billing/plans/actions.test.ts') $run) -ne 0) {
      throw 'Billing plans actions tests failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-plans-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'Billing plan identity, pricing, quotas, subscription details, raw usage-limit values, and plan-change controls remain absent.'
    result = 'PASS'
  })
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    artifactType = 'db-invariant'
    assertion = 'Spec compares non-secret Vendor, TrackingSetting, User, active member VendorMember, BillingPlan, VendorSubscription, and VendorUsageLimit rows plus global, tenant, status, composite, and relation counts; UserSession is verified only by non-secret count and relation projection.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    billingPlansPage = 'Static requireVendorFinance-before-plan/subscription/CSRF/query-use/render contract, five page unit tests, and release-mode Browser denial.'
    ...
    billingPlanAction = 'Six dynamic regression tests cover owner authorization and transactional plan selection; Browser denial does not invoke this action.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp78-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-78 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-78 report: $report"
