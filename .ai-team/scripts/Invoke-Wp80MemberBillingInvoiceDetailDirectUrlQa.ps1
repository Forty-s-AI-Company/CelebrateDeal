...
$snapshot = Join-Path $temp "CelebrateDeal-WP80-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP80-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-80-member-billing-invoice-detail-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-79-member-billing-invoices-direct-url-20260730103420569\final-runner-summary.sanitized.json'
...
function Safe([string]$value) {
  ...
  $safe = $safe -replace '(?i)wp80[-_A-Za-z0-9@./:]+', '[REDACTED_CANARY]'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp80-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp80:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP80-$runId"),
    (Join-Path $temp "CelebrateDeal-WP80-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-80 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31080'
    E2E_BASE_URL = 'http://127.0.0.1:31080'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp80-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp80MemberBillingInvoiceDetailDirectUrlQa.ps1',
    'tests/e2e/member-billing-invoice-detail-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-79 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-79 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/billing/invoices/page.tsx' -Raw
  $guard = $page.IndexOf('const { vendor } = await requireVendorFinance("/billing/invoices");')
  $query = $page.IndexOf('const invoices = await getDb().invoice.findMany({', $guard)
  $issuedTotal = $page.IndexOf('const issuedTotal = invoices.reduce', $query)
  $unpaidTotal = $page.IndexOf('const unpaidTotal = invoices.filter', $issuedTotal)
  $render = $page.IndexOf('title="帳單"', $unpaidTotal)
  if ($guard -lt 0 -or $query -lt $guard -or $issuedTotal -lt $query -or
      $unpaidTotal -lt $issuedTotal -or $render -lt $unpaidTotal) {
    throw 'Billing invoices finance-guard/query/reduction/render order drift.'
  }
  foreach ($required in @(
    ...
    'invoice.dueAt ? formatDateTime(invoice.dueAt) : "未設定"'
  )) {
    if (-not $page.Contains($required)) {
      throw "Billing invoices query/render contract drift: $required"
    }
  }
  ...
  $exportAudit = $export.IndexOf('await writeAuditLog({', $exportQuery)
  $exportResponse = $export.IndexOf('return new Response(`\uFEFF${csv}`', $exportAudit)
  if ($exportGuard -lt 0 -or $exportQuery -lt $exportGuard -or
      $exportAudit -lt $exportQuery -or $exportResponse -lt $exportAudit) {
    throw 'Invoice export guard/query/audit/response order drift.'
  }
  foreach ($required in @(
    'where: { vendorId: vendor.id }',
  ...
  $detail = Get-Content -LiteralPath 'src/app/(app)/billing/invoices/[invoiceId]/page.tsx' -Raw
  $detailGuard = $detail.IndexOf('const { vendor } = await requireVendorFinance("/billing/invoices");')
  $detailParams = $detail.IndexOf('const { invoiceId } = await params;', $detailGuard)
  $detailValidation = $detail.IndexOf('if (!invoiceId || invoiceId.length > 64) notFound();', $detailParams)
  $detailQuery = $detail.IndexOf('const invoice = await getDb().invoice.findFirst({', $detailValidation)
  $detailTenant = $detail.IndexOf('where: { id: invoiceId, vendorId: vendor.id }', $detailQuery)
  $detailMissing = $detail.IndexOf('if (!invoice) notFound();', $detailTenant)
  $detailReceipt = $detail.IndexOf('const isReceipt = invoice.status === "paid";', $detailMissing)
  $detailRender = $detail.IndexOf('<PageHeader', $detailReceipt)
  if ($detailGuard -lt 0 -or $detailParams -lt $detailGuard -or
      $detailValidation -lt $detailParams -or $detailQuery -lt $detailValidation -or
      $detailTenant -lt $detailQuery -or $detailMissing -lt $detailTenant -or
      $detailReceipt -lt $detailMissing -or $detailRender -lt $detailReceipt) {
    throw 'Invoice detail guard/params/validation/tenant-query/render order drift.'
  }
  foreach ($required in @(
    ...
    'invoice.dueAt',
    'invoice.paidAt',
    'invoice.monthlyFeeCents',
    'invoice.overflowFeeCents',
    'invoice.paymentServiceFeeCents',
    'invoice.transactionServiceFeeCents',
    'invoice.affiliateManagementFeeCents',
    'invoice.subtotalCents',
    'invoice.taxCents',
    'invoice.totalCents'
  )) {
    if (-not $detail.Contains($required)) {
      throw "Invoice detail render contract drift: $required"
    }
  }
...
function Preflight {
  if ($schema -notmatch '^wp80_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-80 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-80 ownership inventory does not contain both approved owned paths.'
  }
  ...
  if (Test-NetConnection -ComputerName '127.0.0.1' -Port 31080 -InformationLevel Quiet -WarningAction SilentlyContinue) {
    throw 'WP-80 release port 31080 is already in use.'
  }
  Push-Location $root
  try {
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-80 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp80-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp80-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/member-billing-invoice-detail-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'billing-invoices-detail-unit' $node @('node_modules/vitest/vitest.mjs', 'run', 'src/app/(app)/billing/invoices/[invoiceId]/page.test.tsx') $run) -ne 0) {
      throw 'Billing invoices detail tests failed.'
    }
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/member-billing-invoice-detail-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'request-boundary.sanitized.json' ([ordered]@{
    ...
    exportRequests = 0
    targetDetailGetRequests = 1
    otherDetailRequests = 0
    result = 'PASS'
  })
  Write-Json 'data-disclosure-boundary.sanitized.json' ([ordered]@{
    ...
    deniedPageData = 'The client-supplied target route parameter may appear in the Next.js routing envelope; all DB-derived invoice identity, number, month, line-item amounts, totals, status, dates, detail title, print control, and invoice-specific links remain absent, and the final Dashboard contains no target identifier.'
    result = 'PASS'
  })
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    artifactType = 'db-invariant'
    assertion = 'Spec compares non-secret Vendor, TrackingSetting, User, active member VendorMember, BillingPlan, VendorUsageLimit, and Invoice rows plus global, tenant, status, composite, and relation counts; UserSession is verified only by non-secret count and relation projection.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    artifactType = 'coverage-boundary'
    billingInvoiceDetail = 'Static requireVendorFinance-before-params/validation/tenant-query/render contract and release-mode Browser denial; the request-supplied dynamic route parameter is distinguished from DB-derived disclosure.'
    ...
    dbFixtures = 'Browser fixture and DB equality cover seven non-secret entity classes plus safe UserSession counts and relations.'
    invoiceExportAndIndex = 'Four dynamic regression tests cover tenant CSV export and index links; Browser denial does not request export.'
    invoiceDetail = 'Six expanded dynamic regression cases cover authorized tenant detail rendering and identifiers; Browser denial requests only the single specified detail URL.'
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp80-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-80 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-80 report: $report"
