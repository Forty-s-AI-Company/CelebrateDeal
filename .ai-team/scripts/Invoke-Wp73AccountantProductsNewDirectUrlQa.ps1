...
$snapshot = Join-Path $temp "CelebrateDeal-WP73-$id"
$runtime = Join-Path $temp "CelebrateDeal-WP73-runtime-$id"
$report = Join-Path $root ".ai-team\reports\wp-73-accountant-products-new-direct-url-$id"
$preserveBaselineEvidence = Join-Path $root '.ai-team\reports\wp-72-accountant-interaction-scripts-new-direct-url-20260730071303009\final-runner-summary.sanitized.json'
...
function Assert-TemporaryArtifact([string]$path) {
  ...
  $markerFile = Join-Path $full '.wp73-marker'
...
function Remove-ResidualArtifacts([string]$runId) {
  ...
  $residualMarker = "wp73:$runId"
  $targets = @(
    (Join-Path $temp "CelebrateDeal-WP73-$runId"),
    (Join-Path $temp "CelebrateDeal-WP73-runtime-$runId")
  )
  foreach ($target in $targets) {
    ...
    Write-Output "WP-73 residual cleanup PASS: $full"
  }
}
...
function Get-Environment {
  ...
  return [ordered]@{
    ...
    DIRECT_URL = $url
    E2E_PORT = '31073'
    E2E_BASE_URL = 'http://127.0.0.1:31073'
    ...
    PLAYWRIGHT_JSON_OUTPUT_FILE = (Join-Path $snapshot '.wp73-browser.json')
...
function Get-OwnedPaths {
  return @(
    '.ai-team/scripts/Invoke-Wp73AccountantProductsNewDirectUrlQa.ps1',
    'tests/e2e/accountant-products-new-direct-url.spec.ts'
  )
}
...
function Get-OwnershipInventory([string[]]$status) {
  ...
  if (-not (Test-Path -LiteralPath $preserveBaselineEvidence -PathType Leaf)) {
    throw 'Accepted WP-72 preserve baseline evidence is missing.'
  }
  ...
  if ($null -ne $baselineEvidence.finalRunnerError) {
    throw 'Accepted WP-72 preserve baseline is not a successful run.'
  }
...
function Assert-SourceContract {
  $page = Get-Content -LiteralPath 'src/app/(app)/products/new/page.tsx' -Raw
  $guard = $page.IndexOf('await requireVendorManager();')
  $header = $page.IndexOf('<PageHeader title="新增商品"', $guard)
  $formRender = $page.IndexOf('<ProductForm />', $header)
  if ($guard -lt 0 -or $header -lt $guard -or $formRender -lt $header) {
    throw 'Product new-page guard/render order drift.'
  }
  if (-not $page.Contains('description="建立可綁定到直播間的商品卡與 CTA。"')) {
    throw 'Product new-page description contract drift.'
  }
  ...
  if (-not $shell.Contains('{ href: "/products", label: "商品", icon: Boxes, managerOnly: true }')) {
    throw 'Product navigation manager-only contract drift.'
  }
  ...
  foreach ($required in @(
    ...
    if (-not $checklist.Contains($required)) {
      throw "Dashboard product checklist contract drift: $required"
    }
  }
  $productStart = $checklist.IndexOf('label: "建立商品"')
  $productEnd = $checklist.IndexOf('label: "建立直播間"', $productStart)
  if ($productStart -lt 0 -or $productEnd -lt 0) {
    throw 'Dashboard product checklist boundary missing.'
  }
  if (-not $checklist.Substring($productStart, $productEnd - $productStart).Contains('managerOnly: true')) {
    throw 'Dashboard product checklist is not manager-only.'
  }

  $form = Get-Content -LiteralPath 'src/components/product-form.tsx' -Raw
  foreach ($required in @(
    ...
    if (-not $form.Contains($required)) {
      throw "Product form contract drift: $required"
    }
  }

  $actions = Get-Content -LiteralPath 'src/app/actions.ts' -Raw
  $start = $actions.IndexOf('export async function upsertProductAction')
  $end = $actions.IndexOf('export async function upsertFormAction', $start)
  if ($start -lt 0 -or $end -lt 0) { throw 'Product action boundary missing.' }
  ...
  $update = $slice.IndexOf('getDb().product.update')
  $create = $slice.IndexOf('getDb().product.create')
  $redirect = $slice.IndexOf('redirect("/products");')
  if ($security -lt 0 -or $manager -lt $security -or $idParse -lt $manager -or
      $data -lt $idParse -or $update -lt $data -or $create -lt $update -or
      $redirect -lt $create) {
    throw 'Product action security/manager/input/mutation/redirect order drift.'
  }
  foreach ($required in @(
    ...
    'compareAtCents: optionalText(formData, "compareAtCents")',
    'currency: text(formData, "currency", "TWD")',
    'imageUrl: optionalExternalUrl(formData, "imageUrl"',
    'checkoutUrl: optionalExternalUrl(formData, "checkoutUrl"',
    ...
    if (-not $slice.Contains($required)) {
      throw "Product action tenant/input contract drift: $required"
    }
  }
}

function Preflight {
  if ($schema -notmatch '^wp73_[0-9]+$') { throw 'Invalid schema.' }
  ...
  if (@($status | Where-Object { $_ -in $ownedStatus }).Count -ne 2) {
    throw 'WP-73 owned paths missing or ownership is ambiguous.'
  }
  $inventory = Get-OwnershipInventory $status
  if (@($inventory.owned).Count -ne 2) {
    throw 'WP-73 ownership inventory does not contain both approved owned paths.'
  }
...
if ($PreflightSelfTest) {
  $null = Preflight
  Write-Output 'WP-73 preflight PASS'
  exit 0
}
...
try {
  ...
  try {
    ...
    $bootstrap = Join-Path $snapshot 'wp73-bootstrap.sql'
    [IO.File]::WriteAllText(
      $bootstrap,
      "CREATE SCHEMA IF NOT EXISTS `"$schema`";`nCOMMENT ON SCHEMA `"$schema`" IS '$marker';",
      [Text.UTF8Encoding]::new($false)
    )
    if ((Invoke-Child 'database-bootstrap' $node @($prisma, 'db', 'execute', '--schema', 'prisma/schema.prisma', '--file', '.\wp73-bootstrap.sql') $run) -ne 0) {
    ...
    if ((Invoke-Child 'spec-eslint' $node @('node_modules/eslint/bin/eslint.js', 'tests/e2e/accountant-products-new-direct-url.spec.ts') $run) -ne 0) {
    ...
    if ((Invoke-Child 'browser-e2e' $node @('node_modules/@playwright/test/cli.js', 'test', 'tests/e2e/accountant-products-new-direct-url.spec.ts', '--project=chromium', '--retries=0', '--reporter=json') $run) -ne 0) {
  ...
  Write-Json 'db-invariant-summary.sanitized.json' ([ordered]@{
    ...
    assertion = 'Spec compares complete Vendor, TrackingSetting, User, VendorMember, and active/inactive Product rows plus global, tenant, status, composite, membership, and relation counts; it records no post-authentication POST, non-loopback, or .invalid request.'
    result = 'UNCHANGED'
  })
  Write-Json 'coverage-boundary.sanitized.json' ([ordered]@{
    ...
    productForm = 'Static action/CSRF/field source contract; no dedicated dynamic component suite is claimed.'
    upsertProductAction = 'Static security/manager/input/tenant-mutation contract only; no dedicated dynamic unit coverage is claimed.'
  })
} catch {
  ...
  if ($schemaCreated) {
    try {
      $run = Get-Environment
      $cleanup = Join-Path $snapshot 'wp73-cleanup.sql'
  ...
  try {
    ...
    if (($ownedManifest | ConvertTo-Json -Compress) -ne ($postOwnedManifest | ConvertTo-Json -Compress)) {
      throw 'WP-73 owned content changed during run.'
    }
    if (@(git -C $root diff --cached --name-only).Count -ne 0) {
...
Write-Output "WP-73 report: $report"
